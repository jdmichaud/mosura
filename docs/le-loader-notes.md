...
the RE tracker RE result recorded here.
...
- **Default behaviour = match Ghidra.** A bound DOS/4GW exe (e.g. the subject binary) loads as the
  16-bit MZ stub, exactly as Ghidra does. The committed the subject goldens and the Ghidra-parity
...
- **Opt-in behaviour (`--le` or similar) = the native LE loader.** The user consciously steps
  ...
  path is validated against the **`the RE tracker` reverse-engineering ground truth** (its proper
...
UNTOUCHED). The native-LE path is validated against the the RE tracker RE ground truth (Ghidra has
no LE loader) as a clean subset in `le_subjects_analysis`: 541 functions discovered (watcall cspec
...
**Switch recovery (task #2 — the "beat Ghidra on the subject" win, LANDED).** The *real* protected-mode
...
its MZ-stub `analysis.snapshot (subject profile)` 20 COMPUTED_JUMP — artifacts of misreading the 32-bit code — are not
used). Validated in `le_subjects_analysis` as a clean subset (every COMPUTED_JUMP target mapped, none
...
`SRC FLAGS SRCOFF/CNT OBJECT TARGETOFF [ADDITIVE] [SRCOFF-list]` records). the subject is 100% *internal*
...
default MZ-stub / Ghidra-parity path and its `the subject` goldens are byte-identical.

## Why native, not an ELF32 wrapper

`the RE tracker` gets results today by extracting the LE objects and synthesising a fake ELF32
to feed Ghidra (`tools/ghidra/make_subject_elf.py`). That is a pragmatic workaround for Ghidra's
...
1. **The LE/LX spec** — a documented format, so a faithful implementation is not an
   approximation. Refs (from `~/projects/the RE tracker/README.md`):
...
2. **`the RE tracker` ground truth as the golden** — the human RE work establishes the correct
...
## the subject binary specifics (the reference case)

`~/the subject binary` is **DOS/4GW Pro–bound** (Tenberry). Quirks (from
`~/projects/the RE tracker/analysis/ghidra-setup.md`):
...
- **The inline banner (file `0xAA89C`, right after the `EB 76`) identifies the compiler +
  ...
  grounded in open-watcom-v2 source, validated vs the RE tracker.
...
- Sibling project: `~/projects/the RE tracker` — `analysis/ghidra-setup.md`,
  `tools/ghidra/make_subject_elf.py`, `tools/ghidra/make_subject_objects.py`.
- Memory: (subject-profile note `dos4gw-le`).
