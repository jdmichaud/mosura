...
| `MOSURA_ILV` | behaviour knob (emit) | recompile/recovery.rs:104 recompile/recovery.rs:105 | DELETE the switch and its branch: `=1` enabled the blind interleave form, measured 2026-08-22 as a loser and OFF by default; `interleave_orders` and the `ilv_orders` mark stay as the parked model-inverse groundwork (documented as such; if that leaves them unreachable from any caller they get `#[allow(dead_code)]` with the note, never deleted silently) -- R6 (3), executed |
