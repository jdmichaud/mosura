...
- **The add-fold is one decision point**: `bld/cg/intel/c/i86ver.c`, `V_LEA_GOOD` /
  `V_LEA` — the `OP_ADD`/`OP_SUB` arm returns TRUE (fold to LEA) on any CPU ≥ 286.
  **CORRECTION (2026-08-19):** this entry originally claimed, from black-box probing, that
  OW 1.0's `OptForSize > 50` gate "measurably does not exist in 10.0a or 10.5". That was
...
- **The allocation order is one table**: `386rgtbl.c`'s `DoubleRegs[] = EAX, EDX, ECX,
  ...
  **CHECKED BEFORE PATCHING (2026-08-19) — the claim does not survive its own data, and the
  ...
  1. **The substitutions are near-SYMMETRIC**, which a fixed ordering preference cannot
     produce: `EAX→EDX` 689 vs `EDX→EAX` 675; `EBX→EDX` 426 vs `EDX→EBX` 400; and so on down
     the list. A table reordering predicts a strong one-way bias; the data shows a wash.
  2. **The class is 99.7% entangled**: only **9 functions (40 rows)** have `regalloc` as their
     ONLY divergence class — all of them SAME_SHAPE, sim 0.50–0.85 — while **1,463** carry it
     alongside other classes. Register assignment differences are overwhelmingly DOWNSTREAM
     CASCADE: one differing upstream decision reshuffles every later assignment.

  So "allocation order" is not established as an independent dial at all; most of what the
  row count attributes to it is noise generated by other divergences. Patching `DoubleRegs`
  would have targeted, at most, 40 rows — and the symmetry says it would not have fixed even
  those. **Methodological note:** this is the check the LEA-fold member never got before it
  was believed for months. Row counts measure SYMPTOMS, not independent causes.
