...
| refineRead / refineWrite / refinement / buildRefinement / splitByRefinement / remove13Refinement | PORTED, two scopes. (1) FAITHFUL re-entry `refine_ranges` (heritage.rs; Ghidra `refinement` heritage.cc:1890 from `placeMultiequals` :2608-2616, gate `size>4 && max_write<size`, ≤1024): on a space's re-entry passes (`pass > delay`, the mid-mainloop RuleLoad/StoreVarnode materializations), ALL-SPACE — partition the range by every access boundary (`buildRefinement` :1704 + boundary→size :1911 + `remove13Refinement` :1857), rewrite a free read crossing pieces as a CONCAT of piece reads (`refineRead` :1772/`concatPieces` :507), a write crossing pieces to a unique + per-piece SUBPIECEs with readers re-pointed (`refineWrite` :1806/`splitPieces` :563/`totalReplace`), input-like reads (no dominating write) kept whole (the `refineInput`/`guardInput` realization, mixfloatint). Runs FIRST in `heritage_pass`'s re-entry block (Ghidra order refine :2611 → markers :2627 → normalize :2629); narrow-marker writes excluded (collect's `removevars`, :329-333). Ghidra's disjoint/globaldisjoint rewrite (:1926-1946) needs no analog — refinement precedes `gather_candidates`, so only piece locations are recorded. Healed concatsplit's read-never-written wrong code (0.930→0.978, `CONCAT44(param_6,param_5)` store recovered); 3 synthetic unit tests. (2) Pass-0 batch `refine_overlaps` — laned/XMM-only partition + scalar Normalize (the GP pass-0 partition broadening is its own later gated brick — rule-pool-explosion risk). |
