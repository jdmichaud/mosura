...
| ActionSpacebase | PORTED (pipeline.rs `ActionSpacebase` + `Funcdata::spacebase`, funcdata.cc:230 — task #3 S2a) | Marks every non-free SSA version of the stack pointer RSP `is_spacebase()` + a locked pointer type on the input; wired before the first nonzero-mask/infertypes/pool (coreaction.cc:5506). The `stack` space registers RSP `(register:0x20,8)` as its spacebase register + `space_by_spacebase`/`getSpaceBySpacebase` (architecture.cc:264). Activates the faithful `is_spacebase()` consumers (ptrarith push-guards, nzmask alignment, infertypes pointer propagation). CITATION (varcross −.008): the locked pointer type on RSP exposes mosura's missing `resolveSpacebaseRelative`/stack-array scope — a typed `PTRSUB(spacebase,off)` loop bound stops naming as the stack symbol `axStack_18` (→ undefined `iVar2 + -0x18`); marking-alone is clean (loop pointer becomes oracle-like `pVar1`, no naming loss). Fix folds into S2b/S2c (stack forwarding + resolveSpacebaseRelative). The spacebase-register `RuleLoadVarnode` branch this enables is NOT yet wired (S2b). |
| Funcdata::splitUses | PORTED-UNWIRED (funcdata.rs `split_uses`, funcdata_varnode.cc:1540 — task #27 S1) | Clones a varnode's defining op at each read so every read becomes its own single-use SSA version. The `spacebase()` re-mark arm calls it on an already-spacebase register with an `INT_ADD` def (the frame base `RSP = RSP-0x68`) to turn mosura's ONE broad RSP version — shared by a loop-phi init AND a call arg, hence live across the loop → cover conflict → the #26 `trimOpInput` over-fires the spurious `pVar2 = pVar1;` — into Ghidra's narrow single-use RSP:93/RSP:94. Ported UNWIRED (byte-identical) in S1; the `spacebase()` re-mark arm is S2, the late 2nd-pass wire after reheritage is the S3 mover. 2 unit tests. |
