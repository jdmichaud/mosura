...
| RuleIntLessEqual | DONE — WIRED in the main oppool1 @10 (Task #9) AND the post-orientation `condnegate_pool` (Task #1 S1), faithful to Ghidra's two registrations (coreaction.cc:5521 + the analysis re-run). `V <= c => V < (c+1)` via `replace_lessequal` (port of Funcdata::replaceLessequal), 4 unit tests. Wiring @10 was HELD pre-keystone (it made the print-time negation emit `100 <= x` vs Ghidra `99 < x`); task #8 materialized the negation in the IR → unblocked. Instrumented on wire (Ghidra `capture_trace` on condmulti): the @10 firing is **byte-faithful** — Ghidra fires `intlessequal` on the identical op (`6 <= x => 5 < x`). It exposes ONE downstream gap: mosura lacks **RuleRangeMeld** (coreaction.cc:5612, Task #11), which collapses the SLESS-form flag reconstruction (`(x==c)||(x<c)` jle-form / `(x!=c)&&(c-1<x)` jg-form) that the early SLESSEQUAL→SLESS conversion hands off; mosura had leaned on RuleLessNotEqual (SLESSEQUAL-form only, ruleaction.cc:2290). Until RuleRangeMeld lands, condmulti (0.806→0.750), deindirect (0.857→0.811), elseif (0.904→0.892), loopcomment (0.745→0.687) render the un-collapsed disjunction (corpus 0.9196/56→0.9168/55). The dip is the diagnostic naming that gap, not a mis-port (suite green, switch green) — landed per faithful-ports-land-not-held. printc::incr_in_width already DELETED (mechanism A `ActionPreferComplement` + compound `BlockCondition::negateCondition` materialize the print-time `<=` shortcut in the IR). |
