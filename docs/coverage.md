...
| RulePtrArith | PORTED (ptrarith.rs, ptrarith_pool) — incl. the **TYPE_SPACEBASE arm** of `calcSubtype` (ruleaction.cc:6286, task #22 A): a spacebase pointee always has a matching sub-type (`getSubType` never null), so any `RSP + const` folds to `PTRSUB(RSP, off)`. `hasMatchingSubType` (ruleaction.cc:6064) ports the array-hint path — `TypeSpacebase::nearestArrayedComponent{Backward,Forward}` (type.cc:2971/3020) over the recovered `ScopeLocal` table (`recover_scope`): resolves a variable-indexed stack array to its base + folds the residual into the additive tail. **CITATION (offsetarray −0.314):** the clean `axStack_98[param_1]` subscript needs the *two-pass* PTRADD (the spacebase-PTRSUB output typed as the ScopeLocal array so a 2nd RulePtrArith pass forms `PTRADD(array, i, elem)`) — mosura's reheritage fixpoint group does not run `ActionInferTypes`, so the PTRSUB output is never array-typed; plus a `recover_scope`/SSA offset-convention gap (array placed at -0x98 vs Ghidra's SSA -0x68). Arithmetic is correct (unfolded); the render is not subscripted. = task #22-A-2b (ActionInferTypes-in-reheritage), a separate missing subsystem — faithful exposure, not a mis-port. |
