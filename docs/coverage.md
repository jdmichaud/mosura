...
| RuleIntLessEqual | PARTIAL — WIRED in the post-orientation `condnegate_pool` only (Task #1 S1), NOT in the main oppool1 @10. There it normalizes the freshly-materialized `<=` from RuleCondNegate/RuleBoolNegate (e.g. after a negated guard flips to `<=`). **Two-mechanisms-overlap narrowing (Task #2 unification):** it is NOT wired @10; printc::incr_in_width is now DELETED — mechanism A (`ActionPreferComplement`) + compound (`BlockCondition::negateCondition`) materialize `x <= c => x < c+1` in the IR via `replace_lessequal` for every oriented guard/if-else/compound-leaf, and the audit proved zero corpus fixtures still print-flip a `<`/`<=` (the `x <= c => x < c+1` shortcut is fully in the IR now). What remains @10-unblocked is only wiring RuleIntLessEqual into the main oppool1 @10 (the earlier attempt regressed concat/condconst/condmulti/condsplit into `x == c || x < c`); resolve as Task #2's `<=`-unification (instrument-first; P7/P8 #5/#6). Faithful `replace_lessequal` port of Funcdata::replaceLessequal, 4 unit tests. |
