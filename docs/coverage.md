...
| ActionAssignHigh / MergeRequired / MarkExplicit / MarkImplied / MergeMultiEntry / MergeCopy / DominantCopy / MarkIndirectOnly / MergeAdjacent / MergeType / HideShadow / CopyMarker | PARTIAL (merge.rs, mergesnip.rs) | merge.rs is minimal (high/same/count/merge, read-only naming). **MergeRequired**: the addrtied cover-intersection snip (`Merge::mergeAddrTied`/`unifyAddress`/`eliminateIntersect`/`snipReads` + `characterizeOverlap`/`partialCopyShadow`/`containVarnodeDef`) is PORTED **WIRED** in `mergesnip.rs` (`ActionMergeRequired`), gated on the real ADDRTIED flag (`ActionMarkAddrTied`) so it fires on ram globals / aliased stack slots, not non-aliased stack temps. INDIRECT cover positions come from the `guarded_op`/`op_index` iop link (Ghidra `getUIndex`). partialmerge's snip fires but its snapshot COPY stays flat (printc inlines the single-use COPY) until **MarkExplicit** lands (below) — that is the partialmerge mover. **multiret residual (VERIFY at B-iii):** a partial-overlap snip at a multi-width aliased stack slot renders as `xStack_14 = xStack_14` self-assigns — pre-B-iii inline of a *faithful* partial-overlap snip (matches Ghidra's overlaptype-1 + boundtype-2 logic; multiret is unscored + oracle-uncapturable). Verify at B-iii MarkExplicit: named temps ⇒ resolved; still-spurious ⇒ open a dedicated multi-width-partial-overlap task. **MarkExplicit** = Task #1 B-iii (faithful term-count/depth port, next). Other Merge* actions not individually ported (Task #4 P5). |
