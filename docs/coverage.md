...
| ActionAssignHigh / MergeRequired / MarkExplicit / MarkImplied / MergeMultiEntry / MergeCopy / DominantCopy / MarkIndirectOnly / MergeAdjacent / MergeType / HideShadow / CopyMarker | PARTIAL (merge.rs, mergesnip.rs) | merge.rs is minimal (high/same/count/merge, read-only naming). **MergeRequired**: the addrtied cover-intersection snip (`Merge::mergeAddrTied`/`unifyAddress`/`eliminateIntersect`/`snipReads` + `characterizeOverlap`/`partialCopyShadow`/`containVarnodeDef`) is PORTED **HELD/unwired** in `mergesnip.rs` (`ActionMergeRequired`, unit-tested). Wiring it needs two prereqs first (Task #1 B-i/B-iii): a real ADDRTIED flag set at varnode-props/restructure time to gate it (else the ram/stack space-proxy over-fires on stack SSA inputs), and a faithful `ActionMarkExplicit` port so printc keeps the snapshot COPY explicit (else it inlines the snapshot, defeating partialmerge). Other Merge* actions not individually ported (Task #4 P5). |
