...
| RuleIntLessEqual | PARTIAL — WIRED in the post-orientation `condnegate_pool` only (Task #1 S1), NOT in the main oppool1 @10. There it normalizes the freshly-materialized `<=` from RuleCondNegate/RuleBoolNegate (e.g. after a negated guard flips to `<=`). **Two-mechanisms-overlap narrowing (Task #2 unification):** it is NOT wired @10; printc::incr_in_width is now retired for `if`/`else` (mechanism A's `ActionPreferComplement` materializes `x <= c => x < c+1` in the IR via `replace_lessequal`) but STILL active for the compound `&&`/`||` leaves + residual print-time negation — so mosura still does `x <= c => x < c+1` at print there while doing it in the IR for oriented guards and if/else. Wiring @10 + deleting incr_in_width remains blocked on the same structuring dependency (the earlier attempt regressed concat/condconst/condmulti/condsplit into `x == c || x < c`); resolve as Task #2's `<=`-unification (instrument-first; P7/P8 #5/#6). Faithful `replace_lessequal` port of Funcdata::replaceLessequal, 4 unit tests. |
