...
| ActionReturnRecovery | PORTED (recover.rs) | recover_return / resolve_return. Each return trial passes BOTH of Ghidra's gates (coreaction.cc:1930-1931): realism (`is_realistic` ≈ `AncestorRealistic::execute`) AND USE (`ancestor_op_use` = `Funcdata::ancestorOpUse` + `only_op_use`/`check_call_double_use`/`is_alternate_path_valid`/`TraverseNode` flags): the value must be used *only* to feed the RETURN, not consumed by a STORE/LOAD/BRANCH/CALL/persist elsewhere. The USE gate voids leftover-in-RAX arithmetic (condconst's `&array[i]` STORE address, the merge-bank held global / stack-canary) that realism alone accepts → condconst/piecestruct/pointercmp/etc. become void. mosura's `recover_return` appends several OVERLAPPING candidates (RAX:8/XMM0:8/XMM0:4) where Ghidra registers one output trial per storage, so a float/double return legitimately feeds two sibling candidate slots — `is_return_value_use` counts a use at any return-value slot of the same RETURN as the return use (not external). **BACKLOG (faithful follow-up): retire the overlapping-candidates adaptation.** `is_return_value_use` is the interim faithful accommodation to mosura's multi-candidate representation; the deeper fix is `recover_return` resolving to ONE winning output trial via the active-output `ParamActive`/`characterizeAsOutput` model (Ghidra registers a single output trial), which removes the sibling-slot artifact at its source — likely shared with the call-output side (Task #4 Stage B). Approximations: no typelock/incidental-COPY model; `check_call_double_use` uses block position for `getSeqNum().getOrder()`; single-pass (maxpass 0) so a value that is *computed then both returned and passed to a call* is voided at this stage (the deferred-maxpass/mainloop-repeat class — see the 3 residuals) — not hit by the corpus. |
...
| RuleIntLessEqual | PARTIAL — WIRED in the post-orientation `condnegate_pool` only (Task #1 S1), NOT in the main oppool1 @10. There it normalizes the freshly-materialized `<=` from RuleCondNegate/RuleBoolNegate (e.g. after a negated guard flips to `<=`). **Two-mechanisms-overlap still open (Task #2 unification):** it is NOT wired @10 and printc::incr_in_width is STILL active — so mosura still does `x <= c => x < c+1` at print for un-oriented `<=` while doing it in the IR for oriented guards. Wiring @10 + deleting incr_in_width remains blocked on the same structuring dependency (the earlier attempt regressed concat/condconst/condmulti/condsplit into `x == c || x < c`); resolve as Task #2's `<=`-unification (instrument-first; P7/P8 #5/#6). Faithful `replace_lessequal` port of Funcdata::replaceLessequal, 4 unit tests. |
