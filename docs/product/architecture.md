...
**Phase 2 — `mosura-capi`, the `mosura` binding, and `mosura-cli`.** LANDED 2026-09-05
(`plan-wp7-2026-09-05.md` §3 P2). `crates/mosura-capi` (rlib `mosura_capi` + cdylib + staticlib):
`status` (the ten codes + the per-thread message), `boundary` (`guard`: Err → status, panic →
INTERNAL unless abort-on-panic), `handle` (magic/kind/poison/typed-drop cells behind opaque
pointers, a live-set so a released or foreign pointer is refused before memory is read), `mem`
(views, owned bytes), and one `extern "C"` per header line for §3–§10 (ctx, options, tables,
ops/schema/call, session, language, program, function). `include/mosura.h` is GENERATED by
`cargo xtask header` (cbindgen behind an xtask feature; E11) and committed; the drift test pins
header ↔ exports without cbindgen. `crates/mosura` is the safe binding (handles with `Drop`,
statuses as `Result`, views as slices; no lifetime parameters: the capi holds its own references,
so a session may be dropped before its program). `crates/mosura-cli` is the `mosura` binary over
the binding: the §6.2 tree for the landed operations plus `call`, `-o key=value` projected onto
each operation's parameters, the current program kept in the session config, exit codes
0/2/3/4, a machine config that may hold Environment keys only (none yet; the CLI's one
environment read is `HOME` to find it — the design's `MOSURA_CONFIG` was dropped for
`--config`). Deviations recorded in the plan: `entry` is an address (the CLI resolves names);
`ctx_regs` carries the context settings in its `ctx` key; `mosura_emulate`, `mosura_fingerprint`,
`mosura_program_annotate/unannotate`, `mosura_function_recover`, the IR stages other than `post`
and the function tables other than prototype/jumptables/calls/report answer
MOSURA_ERR_UNSUPPORTED in this version; §11 (toolchains, verify, rounds) arrives with phase 3;
`dumpc` and `trace` stay as dev grounding tools (no pre-pipeline stop, no rule trace op) while
