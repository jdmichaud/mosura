...
2. A number whose @sha != current HEAD is STALE — RE-MEASURE on HEAD (`cargo test -q -p mosura-core --test decompile_corpus decompile_track_corpus_report -- --nocapture`) before using it as a premise. The startup-check report is the ONLY trusted current number.
...
MECHANICAL AID (LANDED e86b7d4): `decompile_track_corpus_report` now stamps `@ <short-sha>` (+ `-dirty` for uncommitted trees) in both header lines — `head_sha()` in crates/mosura-core/tests/decompile_corpus.rs. Every invocation is dated (can't be bypassed); `@sha != HEAD` is a one-glance stale check; `-dirty` catches measuring uncommitted changes (a clean sha over a modified tree is the same trap). So read numbers straight off the report header, sha included.
