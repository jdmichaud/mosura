...
- mosura side: `--debug opaction` env var (OFF by default → corpus byte-identical) hooks ActionPool::apply (action.rs), same format; the alias-probe pool run is suppressed so the trace isn't doubled. Runner = `crates/mosura-core/examples/trace.rs` (TRACKED; dump*.rs stays gitignored).
