...
2. **GOAL = the original bytes.** The scoreboard is the subject recompile — EXACT count, byte-match, band shape. The corpus gauge (mosura vs `oracle/capture --c`) is a DIAGNOSTIC and must never be optimized for its own sake.
...
Related: (subject-profile note `byte-exact-campaign`) · (subject-profile note `stackpointer-rootcause`) · [[faithful-ports-land-not-held]] · [[direction-faithful-port]].
