 ...
 * `docs/function-discovery-backlog.md` §9 #5 — INLINE CALL PARAMETERS DECODED AS CODE, the
 ...
 * parameter and DESTROYS THE NEXT LABEL'S ENTRY. On the war2 MZ stub the destroyed instruction
 * is `00013a56 POP BX`, which the committed Ghidra golden has.
 *
 * WHAT GHIDRA ACTUALLY DOES, measured against `goldens/analysis/war2.snapshot` rather than
 * assumed — and it is NOT a fall-through override:
 *
 *   - Ghidra's listing has NO code unit at any of the four war2 parameter addresses, and no code
 ...
 *
 *  7. The dispatcher's `pop ebx; mov cx,[ebx]` is the war2 idiom verbatim (`5b` / `2e 8b 0f`
