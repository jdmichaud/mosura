...
/// with an EMPTY body and an uninitialized variable; the shape also let the rule pool build a
/// self-referential INT_ADD, which sends `AddExpression::gather` (a faithful port of
/// expression.cc:333, unbounded by design because Ghidra's SSA never holds such an op) into
/// recursion that no stack size absorbs.
///
