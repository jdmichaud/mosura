...
fn caller_cleanup_after(
    ...
    // The scheduler-aware scan lives with the other convention evidence
    // (`convention::caller_stack_cleanup_scan`, unit-tested there); this side only decodes the
    // fallthrough window — 8 instructions bounds the walk well past the 1-2 the scheduler moves.
    ...
    crate::recompile::convention::caller_stack_cleanup_scan(&window, sp)
}
