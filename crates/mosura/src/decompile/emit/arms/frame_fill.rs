...
if let Some(agg) = pr.arms.frame_fill.agg.clone() {
    if agg.covers(sym.start) {
        ...
        ValueSite::OpRoot { .. } | ValueSite::Var { .. } | ValueSite::Equality { .. } | ValueSite::Compare { .. } | ValueSite::Extension { .. } | ValueSite::CallArg { .. } | ValueSite::ReturnValue { .. } | ValueSite::Load { .. } | ValueSite::Sum { .. } | ValueSite::Deref { .. } | ValueSite::VarEntry { .. } => None,
    }
}
