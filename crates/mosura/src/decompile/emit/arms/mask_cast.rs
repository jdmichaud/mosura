...
//! definition and the call (`recovered.mask_sites`, from `buildconfig::masked_args_from_evidence`
...
pub(crate) fn render(pr: &mut PrintC<'_>, op: OpId, slot: usize) -> Option<(String, u8)> {
    ...
    pr.report.mask_candidates.push((call_pc, slot as u32, def_pc, (vn.loc.offset, vn.size)));
    let &width = pr.recovered.mask_sites.get(&(call_pc, slot as u32))?;
