...
pub(crate) struct PrintC<'a> {
    ...
    pub(crate) nonprinting: &'a HashSet<OpId>,
...
impl<'a> PrintC<'a> {
    ...
    pub(crate) fn is_explicit(&self, v: VarnodeId) -> bool {
        let vn = self.f.vn(v);
        if vn.is_constant() {
            return false;
        }
    ...
    pub(crate) fn name_of(&mut self, v: VarnodeId) -> String {
        let vn = self.f.vn(v);
    ...
    pub(crate) fn strlen_arg(&mut self, v: VarnodeId) -> String {
    ...
    fn emit_basic(&mut self, b: super::block::BlockId, indent: usize, out: &mut String) {
        ...
        for op in block_ops.clone() {
            ...
            // struct-copy=assign (emit/arms/struct_copy.rs): a witnessed MOVSD run prints as ONE struct
...
pub(crate) fn render_const_typed(val: u64, size: u32, sign: bool) -> String {
    let masked = if size == 0 || size >= 8 { val } else { val & ((1u64 << (8 * size)) - 1) };
    if sign {
