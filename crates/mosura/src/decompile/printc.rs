...
pub(crate) fn exit_basic(s: &Structured, idx: usize) -> Option<BlockId> {
...
pub(crate) fn entry_basic(s: &Structured, idx: usize) -> Option<BlockId> {
    match &s.blocks[idx].kind {
        FlowKind::Basic(b) => Some(*b),
...
pub(crate) fn operand_oriented(f: &super::funcdata::Funcdata, s: &Structured, idx: usize) -> bool {
...
pub(crate) struct PrintC<'a> {
    ...
    pub(crate) labels: HashSet<BlockId>,
...
impl<'a> PrintC<'a> {
    ...
    pub(crate) fn render_op(&mut self, op: super::op::OpId) -> (String, u8) {
    ...
    pub(crate) fn emit_structured(&mut self, s: &Structured, idx: usize, indent: usize, out: &mut String) {
    ...
    pub(crate) fn next_flow_after(&self, s: &Structured, mut node: usize) -> Option<BlockId> {
        loop {
    ...
    pub(crate) fn plain_if_condition_vn(&self, s: &Structured, if_idx: usize) -> Option<VarnodeId> {
        let fb = &s.blocks[if_idx];
        let FlowKind::Basic(bid) = s.blocks[*fb.components.first()?].kind else { return None };
        let cb = self.f.block(bid).ops.iter().rev().copied().find(|&op| {
            !self.f.op(op).is_dead() && self.f.op(op).code() == OpCode::Cbranch
    ...
    /// with no live ops.
    pub(crate) fn first_pc(&self, s: &Structured, idx: usize) -> Option<u64> {
        let fb = &s.blocks[idx];
