...
struct RepMovs {
    ...
    strlen_result: Option<(VarnodeId, i64)>,
}
...
struct PrintC<'a> {
    ...
    strlen_exprs: HashMap<VarnodeId, (VarnodeId, i64)>,
    ...
    strlen_alias: HashMap<VarnodeId, (VarnodeId, i64)>,
...
impl<'a> PrintC<'a> {
    ...
    /// The `strlen` argument: the resolved loop-pointer entry, cast to `char *` when its own type
    /// is not a char pointer — the entry resolver crossed the CAST `ActionSetCasts` had put on the
    /// phi input (`pcVar2 = (char *)*(xunknown4 *)(p + 0x14)`), and Watcom rejects an integer for
    /// the `const char *` parameter. Ghidra's `castInput` (ActionSetCasts) prints exactly this cast
    /// for a call argument whose type differs from the parameter's.
    fn strlen_arg(&mut self, v: VarnodeId) -> String {
        let (txt, prec) = self.render_var(v);
        if self.type_of(v).name() == "char *" {
            txt
        } else if prec < 14 {
            format!("(char *)({txt})")
        } else {
    ...
    fn strlen_fold(&mut self, op: OpId) -> Option<(String, u8)> {
        if self.strlen_alias.is_empty() {
            return None;
        }
        let o = self.f.op(op);
        let (i0, i1) = (o.input(0)?, o.input(1)?);
        let (av, kv, alias_left) = match (self.strlen_alias.get(&i0), self.strlen_alias.get(&i1)) {
            (Some(&(r, add)), None) if self.f.vn(i1).is_constant() => ((r, add, i0), i1, true),
            (None, Some(&(r, add))) if self.f.vn(i0).is_constant() => ((r, add, i1), i0, false),
            _ => return None,
        };
        let (r, add, v) = av;
        if add == 0 {
        ...
        let size = self.f.vn(kv).size;
        let mask = if size >= 8 { u64::MAX } else { (1u64 << (8 * size)) - 1 };
        let k = self.f.vn(kv).constant_value() & mask;
    ...
    fn try_emit_rep_movs(&mut self, s: &Structured, idx: usize, indent: usize, out: &mut String) -> bool {
        ...
        let stmt = match (info.src, info.set_val) {
            ...
            (None, None) => {
                ...
                if self.strlen_exprs.contains_key(&r) {
                    return true; // implied: rendered at its use, the loop prints nothing
                }
                let r = self.lvalue_of(r);
                let sv = self.strlen_arg(info.dst);
        ...
        match kind {
            ...
            FlowKind::CondAnd | FlowKind::CondOr => {
                ...
                if let Some(eb) = exit_basic(s, idx) {
                    ...
                    // would otherwise print the record again inside the parentheses
                    if let Some(records) = self.gotos.remove(&eb) {
                        let pad = "  ".repeat(indent);
                        for &GotoRecord { target, negated, conditional, is_break } in &records {
                            ...
                            if conditional {
                                let c = self.render_condition(s, idx, negated);
                                let _ = writeln!(out, "{pad}if ({c}) {stmt};");
                            } else {
                                let _ = writeln!(out, "{pad}{stmt};");
                            }
                        }
                        self.gotos.insert(eb, records);
                    }
                }
            }
            FlowKind::Switch => {
            ...
            FlowKind::List => {
                ...
                while i < comps.len() {
                    ...
                    if self.sparse_consumed.contains(&c) {
                        i += 1;
                        continue; // walked into the sparse switch an earlier component printed
                    }
...
impl<'a> PrintC<'a> {
    ...
    fn try_emit_sparse_switch(&mut self, s: &Structured, idx: usize, indent: usize, out: &mut String) -> bool {
        ...
        let root_cond = match s.blocks[cond_node].kind {
            FlowKind::List => s.blocks[cond_node].components.last().and_then(|&c| self.sparse_cbranch_cond(s, c)),
            _ => self.plain_if_condition_vn(s, idx),
        };
        let Some(root_cond) = root_cond else { return false };
        let Some((scrut, _, _, _)) = self.sparse_compare(root_cond) else { return false };
        let scrut_high = self.sparse_key(scrut);
        if std::env::var_os("MOSURA_SPARSE_DEBUG").is_some() {
            eprintln!("[sparse] {:#x} enter root node {idx} key {scrut_high:?} negated {} kind {:?} parent {:?}", self.f.addr.offset, s.blocks[idx].negated, s.blocks[idx].kind, s.blocks[idx].parent.map(|p| (p, format!("{:?}", s.blocks[p].kind), s.blocks[p].components.clone())));
        }
        self.sparse_hoist_pending.borrow_mut().clear();
        self.sparse_exit_goto.borrow_mut().clear();
        self.sparse_head_stmts.borrow_mut().clear();
        self.sparse_root.set(idx);
        ...
        if let Some(parent) = s.blocks[idx].parent {
            ...
            while let Some(a) = anc {
                if matches!(s.blocks[a].kind, FlowKind::If | FlowKind::IfElse) {
                    if let Some(pc) = self.plain_if_condition_vn(s, a) {
                        if self.sparse_compare(pc).is_some_and(|(x, _, _, _)| self.sparse_key(x) == scrut_high) {
                            return false;
                        }
                    }
                    break;
        ...
        let siblings = |parent: Option<usize>| -> Vec<usize> { parent.map(|p| s.blocks[p].components.clone()).unwrap_or_else(|| s.roots.clone()) };
        {
            ...
            if let Some(p) = parent {
                if s.blocks[p].parent.is_some_and(|pp| matches!(s.blocks[pp].kind, FlowKind::If | FlowKind::IfElse) && s.blocks[pp].components.first() == Some(&p)) {
                    if std::env::var_os("MOSURA_SPARSE_DEBUG").is_some() { eprintln!("[sparse]   not a root: inside the condition list of node {:?}", s.blocks[p].parent); }
                    return false;
                }
            }
            if parent.is_none_or(|p| matches!(s.blocks[p].kind, FlowKind::List)) {
                ...
                let compares_scrut = |me: &Self, c: usize| -> bool {
                    let cond = match s.blocks[c].kind {
                        FlowKind::If | FlowKind::IfElse => me.plain_if_condition_vn(s, c),
                        FlowKind::Basic(_) => me.sparse_cond_goto(s, c, true).and_then(|_| me.sparse_cbranch_cond(s, c)),
                        _ => None,
                    };
                    cond.and_then(|v| me.sparse_compare(v)).is_some_and(|(x, _, _, _)| me.sparse_key(x) == scrut_high)
                };
                if std::env::var_os("MOSURA_SPARSE_DEBUG").is_some() {
                    eprintln!("[sparse]   siblings of node {idx} (parent {parent:?}, pos {pos}): {:?}", comps.iter().map(|&c| (c, format!("{:?}", s.blocks[c].kind), compares_scrut(self, c))).collect::<Vec<_>>());
                }
                if comps[..pos].iter().any(|&c| compares_scrut(self, c)) {
                    if std::env::var_os("MOSURA_SPARSE_DEBUG").is_some() { eprintln!("[sparse]   not a root: an earlier sibling compares the scrutinee"); }
                    return false;
                }
                if matches!(s.blocks[idx].kind, FlowKind::If) && comps.get(pos + 1).is_some_and(|&c| compares_scrut(self, c)) {
                    let mut end = comps.len();
                    if end > pos + 2 && self.sparse_is_bare_return(s, comps[end - 1]) {
                        end -= 1;
                    }
        ...
        let size = self.f.vn(scrut).size as i64;
        let signed = self.sparse_signed(root_cond);
        let (lo, hi) = if size >= 8 {
            (i64::MIN / 2, i64::MAX / 2)
        } else if signed {
        ...
        let tail = match list_from {
            Some((parent, _, end)) => {
                let comps = siblings(parent);
                if end < comps.len() { entry_basic(s, comps[end]) } else { self.next_flow_after(s, comps[end - 1]) }
            }
            None => self.next_flow_after(s, idx),
        };
        self.sparse_cond_override_pending.borrow_mut().clear();
        self.sparse_tail.set(tail);
        ...
        if let Some(p) = s.blocks[idx].parent {
            if matches!(s.blocks[p].kind, FlowKind::WhileDo) && s.blocks[p].components.get(1) == Some(&idx) {
                ...
                let cond_vn = match s.blocks[cn].kind {
                    FlowKind::List => s.blocks[cn].components.last().and_then(|&c| self.sparse_cbranch_cond(s, c)),
                    FlowKind::Basic(_) => self.sparse_cbranch_cond(s, cn),
                    _ => None,
                };
                if let Some(cv) = cond_vn {
                    let mut scratch: Vec<OpId> = Vec::new();
                    if let Some(t) = self.sparse_true_ranges(cv, scrut_high, lo, hi, &mut scratch, &mut consts) {
                        ...
                        if std::env::var_os("MOSURA_SPARSE_DEBUG").is_some() { eprintln!("[sparse]   loop condition narrows the root's reach to {:?}", reach.0); }
        ...
        let walked = match list_from {
            Some((parent, pos, end)) => {
                ...
                self.sparse_walk_list(s, &comps, None, scrut_high, signed, reach.clone(), lo, hi, &mut leaves, &mut compares, &mut consts, 0, &mut depth, None)
            }
            None => self.sparse_walk(s, idx, scrut_high, signed, reach.clone(), lo, hi, &mut leaves, &mut compares, &mut consts, 0, &mut depth, None),
        };
        if !walked {
            return false;
        }
        ...
        let folded_eq = |v: i64| -> bool {
            self.recovered.sparse_cmp_sites.iter().any(|(&pc, &(imm, kind, reg))| {
                kind == CMP_EQ
                    && (imm as i64) == v
                    && !consts.iter().any(|&(cp, _, _)| cp == pc)
                    && consts.iter().any(|&(cp, _, _)| pc > cp && pc - cp <= 12 && self.recovered.sparse_cmp_sites.get(&cp).is_some_and(|w| w.2 == reg))
            })
        };
        ...
        if dbg {
            eprintln!("[sparse] {:#x} root node {idx}: scrut key {scrut_high:?} depth {depth} consts {consts:?} tail {tail:?}", self.f.addr.offset);
            for l in &leaves {
                eprintln!("[sparse]   leaf node {} kind {:?} vals {:?}", l.node, s.blocks[l.node].kind, l.vals.0);
            }
        }
        if depth < 2 || leaves.len() < 3 {
            if dbg { eprintln!("[sparse]   bail: depth {depth} leaves {}", leaves.len()); }
            return false;
        }
        ...
        let is_tail = |me: &Self, node: usize| -> bool {
            match s.blocks[node].kind {
                FlowKind::Basic(b) => Some(b) == tail || me.sparse_goto_only(s, node).is_some_and(|t| Some(t) == tail),
                _ => false,
            }
        };
        ...
        let tail_is_return = tail
            ...
            .is_some_and(|tn| self.sparse_is_bare_return(s, tn));
        ...
        let leaf_body = |me: &Self, leaf: &SparseLeaf, goto_target: Option<BlockId>| -> SparseBody {
            if is_tail(me, leaf.node) || goto_target.is_some_and(|t| Some(t) == tail) || (tail_is_return && me.sparse_is_bare_return(s, leaf.node)) {
                SparseBody::Empty
            } else if let Some(t) = goto_target {
                match leaves.iter().find(|l| l.goto.is_none() && me.sparse_goto_only(s, l.node).is_none() && (matches!(s.blocks[l.node].kind, FlowKind::Basic(b) if b == t) || entry_basic(s, l.node) == Some(t))) {
        ...
        for leaf in &leaves {
            ...
            let goto_target = leaf.goto.or_else(|| self.sparse_goto_only(s, leaf.node));
            let tail_like = is_tail(self, leaf.node) || self.sparse_is_bare_return(s, leaf.node) || goto_target.is_some_and(|t| Some(t) == tail);
            ...
            if tail_like && !single {
                ...
                if rest_nonempty {
                    let body = leaf_body(self, leaf, goto_target);
                    match default_body {
                        None => default_body = Some(body),
                        Some(existing) if existing == body => {}
                        Some(existing) => { if dbg { eprintln!("[sparse]   bail: two defaults: {existing:?} vs {body:?} (leaf node {})", leaf.node); } return false; }
                    }
                }
                continue;
            }
            ...
            if !single && !tail_like {
                ...
                if leaf.vals.count() <= 16 && run_bounded(&leaf.vals) {
                    ...
                    let key = leaf_body(self, leaf, goto_target);
                ...
                match default_body {
                    None => default_body = Some(body),
                    Some(existing) if existing == body => {}
                    Some(existing) => { if dbg { eprintln!("[sparse]   bail: two defaults: {existing:?} vs {body:?} (wide body leaf node {})", leaf.node); } return false; }
                }
                continue;
            }
            if !single || (tail_like && !cased_const(leaf.vals.values()[0])) {
                if !tail_like {
                    if dbg {
                        let ops: Vec<String> = match s.blocks[leaf.node].kind { FlowKind::Basic(b) => self.f.block(b).ops.iter().filter(|&&op| !self.f.op(op).is_dead()).map(|&op| format!("{:?}{}", self.f.op(op).code(), if self.f.op(op).is_marker() { "*" } else { "" })).collect(), _ => vec![] };
                        eprintln!("[sparse]   bail: multi-valued non-tail leaf node {} vals {:?} ops {:?} gotos {:?}", leaf.node, leaf.vals.0, ops, s.node_gotos.get(&leaf.node).map(|r| r.len()));
                    }
                    return false;
                }
                let body = leaf_body(self, leaf, goto_target);
        ...
        if dbg { eprintln!("[sparse]   cases {:?} default {:?}", cases.iter().map(|c| (c.0.clone(), c.1, c.2.clone())).collect::<Vec<_>>(), default_body); }
        ...
        for (n, c) in self.sparse_cond_override_pending.borrow_mut().drain(..) {
            self.sparse_cond_override.insert(n, c);
        }
        if let Some((parent, pos, end)) = list_from {
            self.sparse_consumed.extend(siblings(parent)[pos + 1..end].iter().copied());
        }
        ...
        let pc_of = |me: &Self, ci: usize| -> u64 { match cases[ci].1 { SparseBody::Node(n) => me.first_pc(s, n).unwrap_or(0), _ => 0 } };
        order.sort_by_key(|&ci| (matches!(cases[ci].1, SparseBody::Node(_)), pc_of(self, ci)));
        ...
        let join_last: std::collections::HashMap<usize, bool> = {
            ...
            for &ci in &order {
                if let Some(j) = &cases[ci].2 {
                    let last = order.iter().copied().filter(|&o| cases[o].2.as_ref() == Some(j)).max_by_key(|&o| pc_of(self, o)).unwrap();
                    m.insert(ci, last == ci);
                }
            }
            m
        ...
        let scrut_def = self.f.vn(scrut).def;
        let (inline, suppress) = match scrut_high {
            SparseKey::High(h) => {
                let members: Vec<VarnodeId> = self.high_members.get(&h).cloned().unwrap_or_else(|| vec![scrut]);
                let ok = scrut_def.is_some_and(|d| {
                    matches!(self.f.op(d).code(), OpCode::Load | OpCode::Copy | OpCode::IntZext)
                        && members.iter().all(|&m| self.f.vn(m).descend.iter().all(|u| compares.contains(u) || self.f.op(*u).is_dead()))
                });
                (ok, ok)
            }
        ...
        for &op in &compares {
            for i in 0..2 {
                if let Some(v) = self.f.op(op).input(i) {
                    if let Some(d) = self.f.vn(v).def {
                        if self.f.op(d).code() == OpCode::Copy {
                            if let Some(src) = self.f.op(d).input(0) {
        ...
        let copy_uses: Vec<OpId> = copy_srcs.iter().flat_map(|&src| self.f.vn(src).descend.clone()).collect();
        for u in copy_uses {
            let uo = self.f.op(u);
            if uo.code() == OpCode::Copy && !uo.is_dead() {
                if let Some(out) = uo.output {
                    let members: Vec<VarnodeId> = self.high_members.get(&self.high_of[out.0 as usize]).cloned().unwrap_or_else(|| vec![out]);
                    let tree_only = self.f.vn(out).descend.iter().all(|r| compares.contains(r) || self.f.op(*r).is_dead())
                        && members.iter().all(|&m| m == out || self.f.vn(m).descend.iter().all(|r| compares.contains(r) || self.f.op(*r).is_dead()) || !self.f.vn(m).is_written());
                    if tree_only {
                        self.suppressed.insert(u);
                    }
                }
            }
        }
        ...
        match s.blocks[head].kind {
            FlowKind::List => {
                let head_stmts: Vec<usize> = self.sparse_head_stmts.borrow().clone();
            ...
            _ => self.emit_structured(s, head, indent, out),
        }
        let hoisted: Vec<usize> = self.sparse_hoist_pending.borrow_mut().drain(..).collect();
        for c in hoisted {
            self.emit_structured(s, c, indent, out);
        }
        let pad = "  ".repeat(indent);
        let (sv, _) = if inline { self.render_op(scrut_def.unwrap()) } else { self.render_var(scrut) };
        ...
        for (pos, &ci) in order.iter().enumerate() {
            ...
            match body {
                ...
                SparseBody::Goto(t) => {
                    ...
                    let _ = writeln!(out, "{pad}  goto {};", self.lab_name(t));
                }
                SparseBody::Node(n) => {
                    self.emit_structured(s, n, indent + 1, out);
                    if let Some(j) = join {
                        ...
                        if join_last.get(&ci).copied().unwrap_or(true) {
                            for &c in &j {
                                self.emit_structured(s, c, indent + 1, out);
                            }
                            ...
                            let _ = writeln!(out, "{pad}  goto {};", self.lab_name(b));
                        }
                        continue;
                    }
    ...
    fn sparse_key(&self, x: VarnodeId) -> SparseKey {
        let x = strip_copies(self.f, x);
        if let Some(d) = self.f.vn(x).def {
            let o = self.f.op(d);
            match o.code() {
                OpCode::Load => {
                    if let Some(a) = o.input(1) {
                        let a = strip_copies(self.f, a);
                        if let Some(ad) = self.f.vn(a).def {
                            let ao = self.f.op(ad);
                            if matches!(ao.code(), OpCode::Ptradd | OpCode::Ptrsub | OpCode::IntAdd) {
                                if let (Some(b), Some(k)) = (ao.input(0), ao.input(1)) {
                                    if self.f.vn(k).is_constant() {
                                        let mut off = self.f.vn(k).constant_value() as i64;
                                        if ao.code() == OpCode::Ptradd {
                                            off *= o.input(2).map(|e| self.f.vn(e).constant_value() as i64).unwrap_or(1);
                                        }
                                        return SparseKey::Load(self.high_of[strip_copies(self.f, b).0 as usize], off, self.f.vn(x).size);
                                    }
                                }
                            }
                        }
                        return SparseKey::Load(self.high_of[a.0 as usize], 0, self.f.vn(x).size);
                    }
                }
                OpCode::Piece => {
                    if let (Some(hi), Some(lo)) = (o.input(0), o.input(1)) {
                        return SparseKey::Piece(self.high_of[strip_copies(self.f, hi).0 as usize], self.high_of[strip_copies(self.f, lo).0 as usize]);
                    }
                }
                _ => {}
            }
        }
        SparseKey::High(self.high_of[x.0 as usize])
    }
    ...
    fn sparse_compare(&self, cond: VarnodeId) -> Option<(VarnodeId, OpCode, i64, bool)> {
        let mut v = cond;
        let mut neg = false;
        let d = self.f.vn(v).def?;
        if self.f.op(d).code() == OpCode::BoolNegate {
            neg = true;
            v = self.f.op(d).input(0)?;
        }
        let d = self.f.vn(v).def?;
        let o = self.f.op(d);
        let code = o.code();
        ...
        let (a, b) = (o.input(0)?, o.input(1)?);
        let (x, k, mirrored) = if self.f.vn(b).is_constant() {
            (a, self.f.vn(b).constant_value(), false)
        } else if self.f.vn(a).is_constant() {
            (b, self.f.vn(a).constant_value(), true)
        } else {
            return None;
        };
        let size = self.f.vn(x).size;
        ...
        Some((strip_copies(self.f, x), code, kv, neg))
    }
    ...
    fn sparse_compare_mirrored(&self, cond: VarnodeId) -> bool {
        let mut v = cond;
        if let Some(d) = self.f.vn(v).def {
            if self.f.op(d).code() == OpCode::BoolNegate {
                if let Some(i) = self.f.op(d).input(0) {
                    v = i;
                }
            }
        }
        self.f.vn(v).def.and_then(|d| self.f.op(d).input(0)).is_some_and(|a| self.f.vn(a).is_constant())
    }

    fn sparse_signed(&self, cond: VarnodeId) -> bool {
        self.sparse_compare(cond).is_some_and(|(_, c, _, _)| matches!(c, OpCode::IntSless | OpCode::IntSlessequal))
    }

    /// The value ranges for which `cond` (on the scrutinee, domain `[lo, hi]`) is TRUE.
    fn sparse_true_ranges(&self, cond: VarnodeId, scrut_high: SparseKey, lo: i64, hi: i64, compares: &mut Vec<OpId>, consts: &mut Vec<(u64, i64, u8)>) -> Option<Ranges> {
        let Some((x, code, k, neg)) = self.sparse_compare(cond) else {
            if std::env::var_os("MOSURA_SPARSE_DEBUG").is_some() {
                let d = self.f.vn(cond).def.map(|d| { let o = self.f.op(d); (format!("{:x}", o.seqnum.pc.offset), o.code(), (0..o.num_inputs()).map(|i| o.input(i).map(|v| self.f.vn(v).def.map(|dd| format!("{:?}", self.f.op(dd).code())).unwrap_or_else(|| if self.f.vn(v).is_constant() { format!("#{:x}", self.f.vn(v).constant_value()) } else { "in".into() }))).collect::<Vec<_>>()) });
                eprintln!("[sparse]   unreadable compare: cond {cond:?} def {d:?}");
            }
            return None;
        };
        ...
        if self.sparse_key(x) != scrut_high && !neg || self.sparse_key(x) != scrut_high && neg {
            if let Some(ad) = self.f.vn(x).def {
                let ao = self.f.op(ad);
                if ao.code() == OpCode::IntAdd && matches!(code, OpCode::IntLess | OpCode::IntLessequal) {
                    if let (Some(base), Some(cv)) = (ao.input(0), ao.input(1)) {
                        if self.f.vn(cv).is_constant() && self.sparse_key(base) == scrut_high {
                            let size = self.f.vn(base).size as i64;
                            let m = if size >= 8 { u64::MAX } else { (1u64 << (8 * size)) - 1 };
                            let c = (self.f.vn(cv).constant_value() & m) as i64;
                            ...
                            let pc = self.f.vn(cond).def.map(|d| self.f.op(d).seqnum.pc.offset).unwrap_or(0);
                            if std::env::var_os("MOSURA_SPARSE_DEBUG").is_some() { eprintln!("[sparse]   meld compare @{pc:#x}: (x + {c}) {:?} {k} -> [{start}, {end}] neg {neg}", code); }
                            consts.push((pc, start, CMP_LT));
                            consts.push((pc, end, CMP_LE));
                            if let Some(d) = self.f.vn(cond).def {
                                compares.push(d);
                                if let Some(i) = self.f.op(d).input(0) { if let Some(dd) = self.f.vn(i).def { compares.push(dd); } }
                            }
                            compares.push(ad);
        ...
        if self.sparse_key(x) != scrut_high {
            if std::env::var_os("MOSURA_SPARSE_DEBUG").is_some() { eprintln!("[sparse]   key mismatch: compare {:?} k {k} key {:?} vs {scrut_high:?}", code, self.sparse_key(x)); }
            return None;
        }
        ...
        let mirrored = self.sparse_compare_mirrored(cond);
        let (kc, kind) = match self.recovered.sparse_cmp_sites.get(&pc) {
            Some(&(imm, wk, _)) => {
                let size = self.f.vn(x).size;
                let signed = matches!(code, OpCode::IntSless | OpCode::IntSlessequal);
                let mask = if size >= 8 { u64::MAX } else { (1u64 << (8 * size)) - 1 };
        ...
        if std::env::var_os("MOSURA_SPARSE_DEBUG").is_some() { eprintln!("[sparse]   compare @{pc:#x}: x {:?} {k} neg {neg} mirrored {mirrored} -> ({kc}, kind {kind}) witness {:?}", code, self.recovered.sparse_cmp_sites.get(&pc)); }
        consts.push((pc, kc, kind));
        if let Some(d) = self.f.vn(cond).def {
            compares.push(d);
            if let Some(i) = self.f.op(d).input(0) {
                if let Some(dd) = self.f.vn(i).def {
                    compares.push(dd);
                }
            }
        }
    ...
    fn sparse_cond_ranges(&self, s: &Structured, node: usize, scrut_high: SparseKey, lo: i64, hi: i64, compares: &mut Vec<OpId>, consts: &mut Vec<(u64, i64, u8)>) -> Option<Ranges> {
        match s.blocks[node].kind {
            FlowKind::Basic(bid) => {
                // the block must hold nothing but the compare and its branch (the root's head is
                // emitted separately by the caller)
                let cb = self.f.block(bid).ops.iter().rev().copied().find(|&op| !self.f.op(op).is_dead() && self.f.op(op).code() == OpCode::Cbranch)?;
                let cond = self.f.op(cb).input(1)?;
                self.sparse_true_ranges(cond, scrut_high, lo, hi, compares, consts)
            }
            FlowKind::CondAnd | FlowKind::CondOr => {
                ...
                let a = self.sparse_cond_ranges(s, comps[0], scrut_high, lo, hi, compares, consts)?;
                let b = self.sparse_cond_ranges(s, comps[1], scrut_high, lo, hi, compares, consts)?;
                ...
                let (oa, ob) = (operand_oriented(self.f, s, comps[0]), operand_oriented(self.f, s, comps[1]));
                let a2 = if oa ^ fa { a.complement(lo, hi) } else { a.clone() };
                let b2 = if ob ^ fb { b.complement(lo, hi) } else { b.clone() };
                if std::env::var_os("MOSURA_SPARSE_DEBUG").is_some() {
                    eprintln!("[sparse]   cond {:?} node {node}: a {:?} b {:?} oriented ({oa},{ob}) flip ({fa},{fb}) -> a {:?} b {:?}", s.blocks[node].kind, a.0, b.0, a2.0, b2.0);
                }
    ...
    fn sparse_walk(&self, s: &Structured, node: usize, scrut_high: SparseKey, signed: bool, reach: Ranges, lo: i64, hi: i64, leaves: &mut Vec<SparseLeaf>, compares: &mut Vec<OpId>, consts: &mut Vec<(u64, i64, u8)>, d: usize, depth: &mut usize, join: Option<Vec<usize>>) -> bool {
        let _ = signed;
        match s.blocks[node].kind {
            FlowKind::If | FlowKind::IfElse => {
                ...
                let Some((cond, reach)) = self.sparse_if_cond(s, node, reach, scrut_high, signed, lo, hi, leaves, compares, consts, d, depth, join.clone()) else { return false };
                let Some(cond) = cond else {
                    leaves.push(SparseLeaf { node, vals: reach, join: join.clone(), goto: None, exit_goto: self.sparse_exit_goto.borrow().last().copied() });
                    return true;
                };
                ...
                if d > 0 && !self.sparse_cond_accept(s, cond, d) {
                ...
                let Some(t) = self.sparse_cond_ranges(s, cond, scrut_high, lo, hi, compares, consts) else {
                    if std::env::var_os("MOSURA_SPARSE_DEBUG").is_some() {
                        let cv = self.plain_if_condition_vn(s, node);
                        eprintln!("[sparse]   leaf(no ranges) node {node} cond kind {:?} cond vn {:?} compare {:?}", s.blocks[cond].kind, cv, cv.and_then(|c| self.sparse_compare(c)).map(|(x, c, k, n)| (self.high_of[x.0 as usize], c, k, n)));
                    }
                    if d == 0 {
                        return false;
                    }
                ...
                *depth = (*depth).max(d + 1);
                ...
                if let Some(t) = via_goto {
                    self.sparse_exit_goto.borrow_mut().push(t);
                }
                let mut ok = self.sparse_walk(s, comps[1], scrut_high, signed, then_reach, lo, hi, leaves, compares, consts, d + 1, depth, join.clone());
                if ok && comps.len() == 3 {
                    ok = self.sparse_walk(s, comps[2], scrut_high, signed, else_reach.clone(), lo, hi, leaves, compares, consts, d + 1, depth, join.clone());
                }
                if via_goto.is_some() {
                    self.sparse_exit_goto.borrow_mut().pop();
                }
                if !ok {
                    return false;
                }
                if comps.len() != 3 {
                    ...
                    let fallout = via_goto.or_else(|| self.next_flow_after(s, node));
                    if let Some(tb) = fallout {
                        if let Some(tn) = self.sparse_body_node(s, tb) {
            ...
            FlowKind::List => self.sparse_walk_list(s, &s.blocks[node].components.clone(), Some(node), scrut_high, signed, reach, lo, hi, leaves, compares, consts, d, depth, join),
            _ => {
                ...
                if let Some((target, when_true)) = self.sparse_cond_goto(s, node, false) {
                    if self.sparse_pure_cond(s, node) {
                        if let Some(t) = self.sparse_cond_ranges(s, node, scrut_high, lo, hi, compares, consts) {
                            *depth = (*depth).max(d + 1);
                            let taken = if when_true { t } else { t.complement(lo, hi) };
                            let tn = self.sparse_body_node(s, target).unwrap_or(node);
                            ...
                            if let Some(fb) = self.next_flow_after(s, node) {
                                if let Some(fnode) = self.sparse_body_node(s, fb) {
                                    leaves.push(SparseLeaf { node: fnode, vals: rest, join: join.clone(), goto: None, exit_goto: self.sparse_exit_goto.borrow().last().copied() });
                                    return true;
                                }
                            }
                            if std::env::var_os("MOSURA_SPARSE_DEBUG").is_some() { eprintln!("[sparse]   bail: cond-goto node {node} has no structural fall-through"); }
                            return false;
                        }
                    }
                }
                if std::env::var_os("MOSURA_SPARSE_DEBUG").is_some() && matches!(s.blocks[node].kind, FlowKind::CondAnd | FlowKind::CondOr) {
                    eprintln!("[sparse]   leaf(cond group) node {node} kind {:?}: cond-goto {:?} node_gotos {:?} pure {}", s.blocks[node].kind, self.sparse_cond_goto(s, node, true), s.node_gotos.get(&node).map(|r| r.len()), self.sparse_pure_cond(s, node));
                }
                leaves.push(SparseLeaf { node, vals: reach, join: join.clone(), goto: None, exit_goto: self.sparse_exit_goto.borrow().last().copied() });
                true
            }
    ...
    fn sparse_walk_list(&self, s: &Structured, comps: &[usize], list_node: Option<usize>, scrut_high: SparseKey, signed: bool, reach: Ranges, lo: i64, hi: i64, leaves: &mut Vec<SparseLeaf>, compares: &mut Vec<OpId>, consts: &mut Vec<(u64, i64, u8)>, d: usize, depth: &mut usize, join: Option<Vec<usize>>) -> bool {
        ...
        let ok = self.sparse_walk_list_inner(s, comps, list_node, scrut_high, signed, reach, lo, hi, leaves, compares, consts, d, depth, join);
    ...
    fn sparse_walk_list_inner(&self, s: &Structured, comps: &[usize], list_node: Option<usize>, scrut_high: SparseKey, signed: bool, reach: Ranges, lo: i64, hi: i64, leaves: &mut Vec<SparseLeaf>, compares: &mut Vec<OpId>, consts: &mut Vec<(u64, i64, u8)>, d: usize, depth: &mut usize, join: Option<Vec<usize>>) -> bool {
        let dbg = std::env::var_os("MOSURA_SPARSE_DEBUG").is_some();
        let mut cur = reach;
        for (i, &c) in comps.iter().enumerate() {
            ...
            let stmt_leaf = |leaves: &mut Vec<SparseLeaf>, vals: Ranges| match (list_node, i) {
                ...
                _ => {
                    ...
                    leaves.push(SparseLeaf { node: c, vals, join: (!rest.is_empty()).then_some(rest), goto: None, exit_goto: self.sparse_exit_goto.borrow().last().copied() });
                }
            };
            if !is_last && matches!(s.blocks[c].kind, FlowKind::IfElse) {
                ...
                let Some((cond, cur2)) = self.sparse_if_cond(s, c, cur.clone(), scrut_high, signed, lo, hi, leaves, compares, consts, d, depth, join.clone()) else { return false };
                if let Some(cond) = cond {
                    if self.sparse_cond_accept(s, cond, d + 1) {
                        if self.sparse_cond_ranges(s, cond, scrut_high, lo, hi, compares, consts).is_some() {
                            if !self.sparse_walk(s, c, scrut_high, signed, cur2, lo, hi, leaves, compares, consts, d, depth, Some(rest)) {
                                return false;
                            }
                            return true;
                        }
                ...
                if dbg { eprintln!("[sparse]   leaf(list ifelse) list {list_node:?}: comp {i} = node {c}"); }
            ...
            if !is_last && matches!(s.blocks[c].kind, FlowKind::If) {
                ...
                if let Some(cond) = cond {
                    if self.sparse_cond_accept(s, cond, d + 1) {
                        if let Some(t) = self.sparse_cond_ranges(s, cond, scrut_high, lo, hi, compares, consts) {
                            *depth = (*depth).max(d + 1);
                            let then_r = if s.blocks[c].negated { t.complement(lo, hi) } else { t };
                            let then_reach = cur2.intersect(&then_r);
                            if !self.sparse_walk(s, s.blocks[c].components[1], scrut_high, signed, then_reach, lo, hi, leaves, compares, consts, d + 1, depth, join.clone()) {
                                return false;
                            }
            ...
            if matches!(s.blocks[c].kind, FlowKind::List) {
                ...
                return self.sparse_walk_list(s, &flat, inner, scrut_high, signed, cur, lo, hi, leaves, compares, consts, d, depth, join);
            }
            if !is_last {
                ...
                if let Some((target, when_true)) = self.sparse_cond_goto(s, c, false) {
                    if self.sparse_pure_cond(s, c) {
                        if let Some(t) = self.sparse_cond_ranges(s, c, scrut_high, lo, hi, compares, consts) {
                            *depth = (*depth).max(d + 1);
                            let taken = if when_true { t } else { t.complement(lo, hi) };
                            let tn = self.sparse_body_node(s, target).unwrap_or(c);
                            if dbg { eprintln!("[sparse]   cond-goto node {c} -> {target:?} (node {tn}) vals {:?}", cur.intersect(&taken).0); }
                ...
                if dbg {
                    if matches!(s.blocks[c].kind, FlowKind::If | FlowKind::IfElse) {
                        ...
                        let cv = self.plain_if_condition_vn(s, c);
                        eprintln!("[sparse]   unwalked if node {c}: cond node {cn} kind {:?} comps {:?} cond vn {:?} compare {:?} pure {}", s.blocks[cn].kind, s.blocks[cn].components.iter().map(|&x| (x, format!("{:?}", s.blocks[x].kind))).collect::<Vec<_>>(), cv, cv.and_then(|v| self.sparse_compare(v)).map(|(x, code, k, n)| (self.sparse_key(x), code, k, n)), self.sparse_pure_cond(s, cn));
                    }
                    let ops: Vec<String> = match s.blocks[c].kind { FlowKind::Basic(b) => self.f.block(b).ops.iter().filter(|&&op| !self.f.op(op).is_dead()).map(|&op| format!("{:?}{}", self.f.op(op).code(), if self.f.op(op).is_marker() { "*" } else { "" })).collect(), _ => vec![] };
                    eprintln!("[sparse]   leaf(list stmt) list {list_node:?}: comp {i} = node {c} kind {:?} pure {} ops {:?} gotos {:?}", s.blocks[c].kind, matches!(s.blocks[c].kind, FlowKind::If) && self.sparse_pure_cond(s, s.blocks[c].components[0]), ops, s.node_gotos.get(&c).map(|r| r.len()));
                }
                stmt_leaf(leaves, cur);
                return true;
            }
            if !self.sparse_walk(s, c, scrut_high, signed, cur.clone(), lo, hi, leaves, compares, consts, d + 1, depth, join.clone()) {
                return false;
            }
        }
        true
    ...
    fn sparse_if_cond(&self, s: &Structured, if_node: usize, mut reach: Ranges, scrut_high: SparseKey, signed: bool, lo: i64, hi: i64, leaves: &mut Vec<SparseLeaf>, compares: &mut Vec<OpId>, consts: &mut Vec<(u64, i64, u8)>, d: usize, depth: &mut usize, join: Option<Vec<usize>>) -> Option<(Option<usize>, Ranges)> {
        ...
        let last = cc.len() - 1;
        for &c in &cc[..last] {
            let empty_basic = matches!(s.blocks[c].kind, FlowKind::Basic(b) if self.f.block(b).ops.iter().all(|&op| self.f.op(op).is_dead() || self.f.op(op).is_marker()));
            if empty_basic {
                continue;
            }
            if matches!(s.blocks[c].kind, FlowKind::If) {
                let (inner, reach2) = self.sparse_if_cond(s, c, reach.clone(), scrut_high, signed, lo, hi, leaves, compares, consts, d, depth, join.clone())?;
                if let Some(ic) = inner {
                    if self.sparse_pure_cond(s, ic) {
                        if let Some(t) = self.sparse_cond_ranges(s, ic, scrut_high, lo, hi, compares, consts) {
                            *depth = (*depth).max(d + 1);
                            let then_r = if s.blocks[c].negated { t.complement(lo, hi) } else { t };
                            if !self.sparse_walk(s, s.blocks[c].components[1], scrut_high, signed, reach2.intersect(&then_r), lo, hi, leaves, compares, consts, d + 1, depth, join.clone()) {
                                return None;
                            }
            ...
            if self.sparse_pure_stmt(s, c) {
                if d > 0 {
                    self.sparse_hoist_pending.borrow_mut().push(c);
                }
                continue;
            }
            ...
            if d == 0 {
                self.sparse_head_stmts.borrow_mut().push(c);
                continue;
            }
            if std::env::var_os("MOSURA_SPARSE_DEBUG").is_some() {
                let ops: Vec<String> = match s.blocks[c].kind { FlowKind::Basic(b) => self.f.block(b).ops.iter().filter(|&&op| !self.f.op(op).is_dead()).map(|&op| format!("{:?}", self.f.op(op).code())).collect(), _ => vec![] };
                eprintln!("[sparse]   leaf(list cond stmt) if {if_node}: comp node {c} kind {:?} pure_stmt {} gotos {:?} ops {:?}", s.blocks[c].kind, self.sparse_pure_stmt(s, c), matches!(s.blocks[c].kind, FlowKind::Basic(b) if s.gotos.get(&b).is_some()), ops);
            }
            return Some((None, reach));
        }
        self.sparse_cond_override_pending.borrow_mut().push((if_node, cc[last]));
    ...
    fn sparse_cond_accept(&self, s: &Structured, cond: usize, d: usize) -> bool {
        if self.sparse_pure_cond(s, cond) {
            return true;
        }
        if let FlowKind::Basic(bid) = s.blocks[cond].kind {
            ...
            if no_side_effects {
                if d > 0 {
                    self.sparse_hoist_pending.borrow_mut().push(cond);
                }
                return true;
            }
        }
    ...
    fn sparse_pure_cond(&self, s: &Structured, node: usize) -> bool {
        match s.blocks[node].kind {
            FlowKind::Basic(bid) => {
                let ops: Vec<OpId> = self.f.block(bid).ops.iter().copied().filter(|&op| !self.f.op(op).is_dead() && !self.f.op(op).is_marker()).collect();
                ...
                let pure = ops.iter().all(|&op| {
                    ...
                    o.output.map_or(true, |out| self.f.vn(out).descend.iter().all(|u| ops.contains(u) || self.f.op(*u).is_dead()))
                });
                if !pure && std::env::var_os("MOSURA_SPARSE_DEBUG").is_some() {
                    eprintln!("[sparse]   impure cond block {bid:?}: {:?}", ops.iter().map(|&op| self.f.op(op).code()).collect::<Vec<_>>());
                }
                pure
            }
            FlowKind::CondAnd | FlowKind::CondOr => s.blocks[node].components.iter().all(|&c| self.sparse_pure_cond(s, c)),
            _ => false,
        }
    }

    /// A basic-block node whose only content is an unconditional goto: its target.
    fn sparse_goto_only(&self, s: &Structured, node: usize) -> Option<BlockId> {
        let FlowKind::Basic(bid) = s.blocks[node].kind else { return None };
        let live = self.f.block(bid).ops.iter().any(|&op| !self.f.op(op).is_dead() && !self.f.op(op).is_marker() && self.f.op(op).code() != OpCode::Branch);
        if live {
            return None;
        }
    ...
    fn sparse_body_node(&self, s: &Structured, b: BlockId) -> Option<usize> {
        let root = self.sparse_root.get();
        let tail = self.sparse_tail.get();
    ...
    fn sparse_cbranch_cond(&self, s: &Structured, node: usize) -> Option<VarnodeId> {
        let FlowKind::Basic(bid) = s.blocks[node].kind else { return None };
        let cb = self.f.block(bid).ops.iter().rev().copied().find(|&op| !self.f.op(op).is_dead() && self.f.op(op).code() == OpCode::Cbranch)?;
        self.f.op(cb).input(1)
    }
    ...
    fn sparse_cond_goto(&self, s: &Structured, node: usize, allow_break: bool) -> Option<(BlockId, bool)> {
    ...
    fn sparse_is_bare_return(&self, s: &Structured, node: usize) -> bool {
        let FlowKind::Basic(bid) = s.blocks[node].kind else { return false };
        // the epilogue's register restores are COPYs that never print; the statement is `return;`
        let live: Vec<OpId> = self.f.block(bid).ops.iter().copied().filter(|&op| !self.f.op(op).is_dead() && !self.f.op(op).is_marker() && self.f.op(op).code() != OpCode::Copy).collect();
        live.len() == 1 && self.f.op(live[0]).code() == OpCode::Return
    }
}
