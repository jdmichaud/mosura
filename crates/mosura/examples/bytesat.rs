//! Throwaway grounding tool: print bytes from the LOADED (LE-fixup-applied) the subject image at a VA.
//!
...
//!
//! Usage: `cargo run -q --release --example bytesat -- <the subject.exe> <va-hex> [len]`
use mosura::analysis;
use mosura::decompile::space::Address;

fn main() {
    let mut args = std::env::args().skip(1);
    let bin = args.next().expect("the subject.exe path");
