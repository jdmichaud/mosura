...
//!
//! Run: `cargo test --release -p mosura-core --test ground_truth_recompile -- --ignored`
//!
...
use mosura_core::recompile::groundtruth::{default_workers, gcc_available, gcc_programs, recompile_programs, EmitPlan, GtReport, GtTimings, Target};
...
fn decompile_recompile_does_not_regress_against_the_local_baseline() {
    ...
    let workdir = mosura_core::paths::workspace_root().join("build/gt-recompile");
    ...
    let update = mosura_core::devcfg::gt_update_baseline(); // `gt.update_baseline = true` in dev-config.toml
