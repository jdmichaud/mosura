...
use mosura_core::decompile::printc::{print_c_recovered, print_c_report, RecoveredChoices};
...
fn rep_movsd_renders_memcpy_when_witnessed() {
    ...
    let (spec, ctx) = mosura_core::lang::load_cached(lang_id).expect("x86 SLEIGH tables load");
    ...
    let sites =
        mosura_core::recompile::buildconfig::string_ops_from_evidence(&report.string_ops.candidates, &insns);
    ...
    let recovered = RecoveredChoices { string_ops: mosura_core::decompile::emit::arms::string_ops::Sites { sites: sites, ..Default::default() }, ..Default::default() };
    ...
    let loop_c = mosura_core::decompile::printc::print_c(&f);
...
fn rep_movs_pair_renders_one_memcpy() {
    ...
    let insns = normalize("x86:LE:32:default", &dt.chunks[0].bytes, entry, &NoReloc).unwrap_or_default();
    let sites = mosura_core::recompile::buildconfig::string_ops_from_evidence(&report.string_ops.candidates, &insns);
