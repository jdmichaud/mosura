...
impl OperandAddressQuery for ProgramOperandAddresses<'_> {
    fn operand_is_address(
        ...
        op_index: usize,
        objects: &[mosura_core::sleigh::OpObject],
    ) -> bool {
        self.0.reference_manager.references().any(|r| {
            ...
            objects.iter().any(|o| {
                matches!(o, mosura_core::sleigh::OpObject::Scalar { signed_value }
...
fn hash_function(program: &Program, golden: &GoldenFunction) -> Option<FidHashQuad> {
    let (spec, ctx) = mosura_core::lang::load_cached(&program.language_id)?;
    ...
    for &(min, max) in &golden.ranges {
        ...
        let addr = mosura_core::decompile::space::Address::new(program.default_space, min);
    ...
    let units: Vec<CodeUnitInput> = decoded
        ...
        .map(|(insn, fp)| {
            ...
            let addr = mosura_core::decompile::space::Address::new(program.default_space, insn.address);
            let next = insn.address + insn.bytes.len() as u64;
            let props = mosura_core::analysis::flowtype::overridden_flow_props(
                &insn.ops,
                insn.address,
                next,
