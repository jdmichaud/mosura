...
use mosura_core::analysis::{self, snapshot};
use mosura_core::conformance::Tally;
use mosura_core::paths::{analysis_corpus_dir, analysis_goldens_dir, cnv_exe, comcom32_exe};
...
fn cached_analyze(path: &std::path::Path) -> std::sync::Arc<mosura_core::analysis::program::Program> {
    use std::collections::HashMap;
    use std::sync::{Arc, Mutex, OnceLock};
    static CACHE: OnceLock<Mutex<HashMap<PathBuf, Arc<mosura_core::analysis::program::Program>>>> =
        OnceLock::new();
...
fn corpus() -> Vec<(String, PathBuf, bool, PathBuf)> {
    ...
    for s in mosura_core::devcfg::subjects() {
...
fn pe_compiler_opinion_borland() {
    let Some(path) = mosura_core::devcfg::binary("bc45") else {
...
fn pe_compiler_opinion_msvc() {
    let Some(path) = mosura_core::devcfg::binary("vc6") else {
...
fn compiler_version_committed_fixtures() {
    use mosura_core::analysis::loader::compiler_version::{detect, Family, Precision};
...
fn compiler_version_proprietary_fixtures() {
    use mosura_core::analysis::loader::compiler_version::{detect, Family};
    ...
    for (key, fam, label) in cases {
        let Some(path) = mosura_core::devcfg::binary(key) else {
...
fn compiler_version_marker_fragments() {
    use mosura_core::analysis::loader::compiler_version::detect;
...
fn le_subjects_analysis() {
    ...
    for s in mosura_core::devcfg::subjects() {
        ...
        let at = |o: u64| mosura_core::decompile::space::Address::new(ram, o);
        ...
        for (k, v) in s.expectations().entries() {
            let Some(from) = k.strip_prefix("le.dispatch_").and_then(mosura_core::devcfg::parse_u64) else { continue };
            let want = mosura_core::devcfg::parse_u64_list(v).unwrap_or_else(|| panic!("subject {id}: `{k}` is not a list of addresses"));
...
fn watcom_detection() {
    ...
    let info = mosura_core::analysis::loader::watcom::detect(&data).expect("watcom banner in fixture");
    ...
    let prog = mosura_core::analysis::loader::load(&data).expect("load watcom_hello.exe");
    ...
    for s in mosura_core::devcfg::subjects() {
        ...
        assert_eq!(mosura_core::analysis::loader::load(&d).unwrap().compiler, expected, "subject {}: default dispatch", s.id);
        if s.expect("le.entry").is_some() {
            assert_eq!(mosura_core::analysis::loader::load_le(&d).unwrap().compiler, expected, "subject {}: LE dispatch", s.id);
        }
    ...
    if comcom.exists() {
        ...
        assert!(mosura_core::analysis::loader::watcom::detect(&d).is_none(), "DJGPP must not match Watcom");
        assert_eq!(mosura_core::analysis::loader::load(&d).unwrap().compiler, "unknown", "non-Watcom → unknown");
...
fn le_subjects_objects() {
    use mosura_core::analysis::loader;
    use mosura_core::analysis::program::SymbolType;
    let mut ran = 0;
    for s in mosura_core::devcfg::subjects() {
        ...
        if let Some(entry) = s.expect_u64("le.entry") {
            ...
            if let Some(bytes) = s.expect("le.entry_bytes") {
                ...
                let got: Vec<Option<u8>> = (0..want.len() as u64)
                    .map(|i| prog.memory.byte_at(mosura_core::decompile::space::Address::new(e.space, entry + i)))
                    .collect();
