...
//!
//! Run: `cargo test --release -p mosura-core --test recompile_toolchain -- --ignored`
//!
...
fn watcom(tag: &str) -> CompilerDriver {
    let dir = mosura_core::paths::watcom_dir();
    assert!(
...
fn compiles_a_batch_and_reports_each_unit() {
    ...
    let cand = mosura_core::recompile::load_object_function(obj, "add2_", 0x1000, &(|_: &str| None))
        .expect("extract add2");
