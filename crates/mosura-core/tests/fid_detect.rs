...
use mosura_core::analysis::fid::detect::detect_version;
...
fn a_vc6_binary_is_dated_to_visual_studio_1998() {
    ...
    let program = mosura_core::analysis::analyze_file(&binary).expect("analyze");
...
fn the_vote_answers_the_family_question() {
    ...
    if unrelated.exists() {
        let program = mosura_core::analysis::analyze_file(&unrelated).expect("analyze");
        let report = detect_version(&program, &dir);
...
fn signature_detection_refines_without_overriding() {
    ...
    let mut program = mosura_core::analysis::analyze_file(&binary).expect("analyze");
    let before_compiler = program.compiler.clone();
    let before_version = program.compiler_version.clone();

    let label = mosura_core::analysis::fid::detect::apply_signature_detection(&mut program, &dir);
...
fn signature_detection_is_silent_when_unsure() {
    ...
    let mut program = mosura_core::analysis::analyze_file(&binary).expect("analyze");
    assert_eq!(mosura_core::analysis::fid::detect::apply_signature_detection(&mut program, &dir), None);
