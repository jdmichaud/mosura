...
use mosura_core::analysis::fid::analyzer::{self, FidAnalyzer};
use mosura_core::analysis::fid::query::FidQueryService;
use mosura_core::analysis::analyzer::Analyzer;
use mosura_core::paths;
...
fn identify(binary: &std::path::Path) -> Option<(BTreeSet<String>, usize, usize)> {
    ...
    let program = mosura_core::analysis::analyze_file(binary).expect("analyze the probe");
...
fn analyzer_is_inert_without_a_database() {
    ...
    let mut program = mosura_core::analysis::analyze_file(&path).expect("analyze");
    ...
    let set = mosura_core::analysis::program::AddressSet::new();
    let mut sched = mosura_core::analysis::manager::Scheduling::default();
...
fn analyze_applies_library_names_end_to_end() {
    ...
    let program = mosura_core::analysis::analyze_file(&path).expect("analyze");
...
fn ambiguous_matches_leave_a_plate_comment_without_renaming() {
    use mosura_core::analysis::fid::matcher::apply_markup;
...
fn ambiguous_search_result() -> Option<mosura_core::analysis::fid::matcher::SearchResult> {
    use mosura_core::analysis::fid::matcher::{HashFamily, Seeker};
    let dir = paths::fid_db_dir();
    if !dir.exists() {
        return None;
    }
    ...
    groups.sort_by_key(|(hash, _)| **hash);
    {
        for (hash, recs) in groups {
            ...
            let quad = mosura_core::analysis::fid::hash::FidHashQuad {
            ...
            if let Some(r) = seeker.process_matches(&HashFamily { hash: Some(quad), ..Default::default() }) {
                if mosura_core::analysis::fid::matcher::apply_markup(&r).name.is_none() {
                    return Some(r);
                }
            }
        }
