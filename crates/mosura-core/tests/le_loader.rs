...
fn auto_analysis_discovers_the_planted_call_graph() {
    for &(stub, entry) in CASES {
        ...
        mosura_core::analysis::analyze(&mut prog);
        let found: Vec<u64> =
...
fn native_dispatch_routes_le_and_the_default_stays_on_the_stub() {
    ...
    assert_eq!(mosura_core::analysis::native_loader_name(&data), Some("LE"));
...
fn bound_and_standalone_are_distinguished() {
    ...
    assert_eq!(mosura_core::analysis::native_loader_name(&bound), Some("LE"));
    assert_eq!(mosura_core::analysis::native_loader_name(&standalone), Some("LE"));
