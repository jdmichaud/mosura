...
use mosura_core::analysis::analyzers::thunk::{self, Candidate, Outcome};
use mosura_core::analysis::program::{Program, RefType};
...
fn report_of(program: &Program) -> Vec<Candidate> {
    let (spec, ctx) =
        mosura_core::lang::load_cached(&program.language_id).expect("SLEIGH tables for the program");
