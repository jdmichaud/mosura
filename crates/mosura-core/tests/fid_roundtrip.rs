...
use mosura_core::analysis::fid::analyzer::{hash_function, search_program};
use mosura_core::analysis::fid::ingest::{Ingest, IngestFunction};
use mosura_core::analysis::fid::matcher::{SCORE_THRESHOLD, SPECIFIC_SCORE_WEIGHT};
use mosura_core::analysis::fid::query::FidQueryService;
use mosura_core::analysis::fid::store::FidStore;
...
fn round_trip(stem: &str) -> Option<(BTreeSet<String>, BTreeSet<String>)> {
    ...
    let program = mosura_core::analysis::analyze_file(&binary).ok()?;
...
fn a_database_does_not_match_unrelated_code() {
    ...
    let source_program = mosura_core::analysis::analyze_file(&source).expect("analyze source");
    ...
    let target_program = mosura_core::analysis::analyze_file(&target).expect("analyze target");
