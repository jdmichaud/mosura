...
fn compare_tree_renders_as_the_sparse_switch() {
    ...
    let insns = mosura_core::recompile::insn::normalize(lang_id, dt.chunks[0].bytes.as_slice(), entry, &mosura_core::recompile::insn::NoReloc).unwrap_or_default();
    let recovered = RecoveredChoices { sparse_switch: mosura_core::decompile::emit::arms::sparse_switch::Sites { sites: mosura_core::recompile::buildconfig::sparse_cmps_from_evidence(&insns), ..Default::default() }, ..Default::default() };
