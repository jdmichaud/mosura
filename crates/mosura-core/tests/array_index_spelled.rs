...
use mosura_core::decompile::printc::{print_c, print_c_recovered, print_c_report, RecoveredChoices};
use mosura_core::decompile::{build, pipeline};
use mosura_core::recompile::buildconfig::array_index_sites_from_evidence;
...
fn witnessed_scaled_index_access_spells_as_a_subscript() {
    ...
    let (spec, ctx) = mosura_core::lang::load_cached(lang_id)
    ...
    let recovered = RecoveredChoices { array_index: mosura_core::decompile::emit::arms::array_index::Sites { sites: sites, ..Default::default() }, ..Default::default() };
