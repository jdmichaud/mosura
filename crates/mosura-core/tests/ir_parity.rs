...
use mosura_core::decompile::build::raw_funcdata_flow;
...
fn x86_64() -> Option<(&'static Spec, Vec<u32>)> {
    ...
    let spec = mosura_core::speccache::get(&sla)?;
...
fn ghidra_ir(fixture: &std::path::Path, action: &str) -> Option<String> {
    ...
    let r = mosura_core::oraclecache::capture(fixture, &["--ir", action]);
    match r {
        None => {
...
fn mosura_block_ranges(spec: &Spec, ctx: &[u32], fixture: &std::path::Path) -> Vec<(u64, u64)> {
    ...
    let mut got: Vec<(u64, u64)> = (0..f.num_blocks() as u32)
        .filter_map(|b| f.block_range(mosura_core::decompile::BlockId(b)))
        .collect();
    got.sort_unstable();
    got
}
...
fn heritaged(spec: &Spec, ctx: &[u32], fixture: &std::path::Path) -> mosura_core::decompile::Funcdata {
    let dt = datatest::parse_file(fixture).expect("fixture");
    let mut f = raw_funcdata_flow(spec, "func", &dt.chunks[0].bytes, dt.chunks[0].offset, ctx);
    mosura_core::decompile::cfg::build_cfg(&mut f);
    let dom = mosura_core::decompile::dominator::compute(&f);
    let _ = mosura_core::decompile::heritage::heritage(&mut f, &dom);
    f
}

#[test]
fn heritage_produces_valid_ssa() {
    use mosura_core::decompile::{OpCode, OpId};
    let Some((spec, ctx)) = x86_64() else { return };
    ...
    for name in ["x86_64_sem", "elseif", "twodim", "threedim"] {
        ...
        for b in 0..f.num_blocks() as u32 {
            for &op in &f.block(mosura_core::decompile::BlockId(b)).ops {
                let o = f.op(op);
        ...
        for i in 0..f.num_varnodes() as u32 {
            let vid = mosura_core::decompile::VarnodeId(i);
        ...
        if name == "twodim" || name == "threedim" {
            ...
            for i in 0..f.num_varnodes() as u32 {
                let vn = f.vn(mosura_core::decompile::VarnodeId(i));
        ...
        let count_phi = |f: &mosura_core::decompile::Funcdata| {
        ...
        for b in 0..f.num_blocks() as u32 {
            let blk = mosura_core::decompile::BlockId(b);
...
fn rule_pool_folds_constants() {
    use mosura_core::decompile::action::{Action, ActionPool};
    use mosura_core::decompile::rules::{eval_const, RuleCollectTerms, RuleConstFold, RuleEarlyRemoval, RuleIdentityEl, RulePropagateCopy, RuleTermOrder, RuleTrivialArith, RuleTrivialShift};
    use mosura_core::decompile::{OpCode, OpId};
    let Some((spec, ctx)) = x86_64() else { return };
...
fn merge_groups_phi_versions_into_variables() {
    use mosura_core::decompile::merge::merge;
    use mosura_core::decompile::{pipeline, OpCode};
    let Some((spec, ctx)) = x86_64() else { return };
    ...
    for name in ["threedim", "elseif", "twodim"] {
        ...
        let mut live_vns: BTreeSet<mosura_core::decompile::VarnodeId> = BTreeSet::new();
        for b in 0..f.num_blocks() as u32 {
            for &op in &f.block(mosura_core::decompile::BlockId(b)).ops {
                let o = f.op(op);
                ...
                if o.code() == OpCode::Multiequal {
                    ...
                    let tied_like = |v: mosura_core::decompile::VarnodeId| {
...
fn merged_variables_have_no_internal_interference() {
    use mosura_core::decompile::cover::all_covers;
    use mosura_core::decompile::merge::merge;
    use mosura_core::decompile::{pipeline, OpCode, VarnodeId};
    ...
    for name in ["x86_64_sem", "twodim", "threedim", "elseif"] {
                ...
                for j in (i + 1)..members.len() {
                    assert!(
                        !covers[&members[i]].intersects(&covers[&members[j]])
                            || mosura_core::decompile::merge::copy_shadow(&f, members[i], members[j]),
        ...
        let had_phi = (0..f.num_blocks() as u32).any(|b| {
            f.block(mosura_core::decompile::BlockId(b))
                .ops
                .iter()
...
fn structuring_collapses_reducible_cfgs() {
    use mosura_core::decompile::structure::structure;
    let Some((spec, ctx)) = x86_64() else { return };
...
fn stack_recovery_collapses_the_frame() {
    use mosura_core::decompile::{pipeline, printc::print_c, OpId};
    let Some((spec, ctx)) = x86_64() else { return };
