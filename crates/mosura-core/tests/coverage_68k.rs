...
use mosura_core::{datatest, golden, paths};
...
fn m68k_disasm_pcode_coverage() {
    ...
    let spec = mosura_core::speccache::get(&sla).expect("68000 spec");
...
fn m68k_coldfire_matches_default_variant() {
    ...
    let cold = mosura_core::speccache::get(&cold_p).expect("coldfire spec");
    let dflt = mosura_core::speccache::get(&dflt_p).expect("68040 spec");
    ...
    for entry in std::fs::read_dir(paths::ground_truth_dir()).expect("ground-truth dir") {
        ...
        programs += 1;
        let render = |i: &mosura_core::sleigh::Instruction| format!("{} {}", i.mnemonic.trim(), i.body);
