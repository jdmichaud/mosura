...
use mosura_core::recompile::recovery::{measured_arms, recover};
...
fn decompiled(fixture: &str) -> (Funcdata, Vec<NormInsn>) {
    ...
    f.tail_return_write = mosura_core::recompile::buildconfig::tail_return_write_from_evidence(&insns);
...
fn ret_n_without_parameters_declares_dummy_stack_parameters() {
    ...
    assert_eq!(mosura_core::recompile::buildconfig::dummy_stack_params(&f), 1);
...
fn far_return_is_witnessed_by_retf() {
    ...
    assert!(mosura_core::recompile::buildconfig::far_return_from_evidence(&insns));
}
