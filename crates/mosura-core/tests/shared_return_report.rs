...
use mosura_core::analysis::analyzers::shared_return::SharedReturnAnalyzer;
use mosura_core::analysis::program::{AddressSet, Program};
use mosura_core::decompile::space::Address;
use mosura_core::paths::{analysis_corpus_dir, comcom32_exe};
...
/// itself the finding.
fn watch_of(s: &mosura_core::devcfg::Subject) -> Vec<u64> {
...
fn report_all(program: &Program) -> Option<(SharedReturnAnalyzer, mosura_core::analysis::analyzers::shared_return::ScanReport)>
{
...
fn subjects_le_shared_return_report() {
    for s in mosura_core::devcfg::subjects() {
        ...
        let prog = mosura_core::analysis::analyze_le_file(&s.path).expect("native-LE analysis of the subject");
...
fn subjects_mz_shared_return_report() {
    for s in mosura_core::devcfg::subjects() {
        ...
        let prog = mosura_core::analysis::analyze_file(&s.path).expect("MZ analysis of the subject");
...
fn corpus_shared_return_report() {
    ...
    for (name, path) in fixtures {
        ...
        let prog = mosura_core::analysis::analyze_file(&path).expect("analysis of the corpus fixture");
