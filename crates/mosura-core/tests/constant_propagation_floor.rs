...
//!
//! The cost lives one level below the analyzer, in [`mosura_core::analysis::symbolic::flow_constants`],
...
use mosura_core::analysis::analyzers::ConstantPropagationAnalyzer;
use mosura_core::analysis::manager::Scheduling;
use mosura_core::analysis::program::{AddressSet, Program};
use mosura_core::decompile::space::Address;
use mosura_core::paths::analysis_corpus_dir;
...
fn constant_propagation_cost_tracks_the_walk_not_a_per_location_floor() {
    ...
    let mut program = mosura_core::analysis::analyze_file(&path).expect("analyze mingw_hello.exe");
