...
use mosura_core::analysis::loader::{self, x32};
...
fn maps_one_flat_block_and_the_entry() {
    for &(stub, sixteen, entry) in CASES {
        ...
        let at = |a: u64| program.memory.byte_at(mosura_core::decompile::space::Address::new(b.start().space, a));
...
fn auto_analysis_discovers_the_planted_call_graph() {
    ...
    for &(stub, sixteen, entry) in CASES {
        ...
        mosura_core::analysis::analyze(&mut program);
...
fn native_dispatch_routes_x32_and_declines_others() {
    ...
    assert_eq!(mosura_core::analysis::native_loader_name(&data), Some("X-32"));
    ...
    bare_mz[0x08..0x0a].copy_from_slice(&2u16.to_le_bytes());
    assert_eq!(
        mosura_core::analysis::native_loader_name(&bare_mz),
        None,
...
fn real_x32_binary_analyses_cleanly() {
    let path = mosura_core::paths::x32_exe();
    ...
    let l = x32::detect_x32(&data).expect("layout");
    let program = mosura_core::analysis::analyze_native_file(&path).expect("native analysis");
