...
fn big_endian_language_marks_its_spaces_big_endian() {
    for id in ["PowerPC:BE:32:default", "MIPS:BE:32:default", "68000:BE:32:Coldfire"] {
        let Some((spec, _)) = mosura_core::lang::load_cached(id) else {
        ...
        let f = build::raw_funcdata_flow_image_arch(spec, "e", &image, 0x1000, mosura_core::lang::load_cached(id).unwrap().1, &format!("{id}:default"));
...
fn big_endian_function_decompiles() {
    let Some((spec, ctx)) = mosura_core::lang::load_cached("PowerPC:BE:32:default") else {
...
fn justified_contain_flips_with_endianness() {
    use mosura_core::decompile::space::{Address, SpaceManager};
