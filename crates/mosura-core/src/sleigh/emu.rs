...
impl Machine {
    ...
    fn step(&mut self, op: &PcodeOp) -> Flow {
        ...
        let res: u64 = match opname {
            ...
            // Signed overflow is a fact about the OPERANDS' width. The operands arrive
            // sign-extended to i64 (`sa`), so the sum is re-masked and re-sign-extended to the
            // input width before its sign is read: `sext(mask(_,8),8)` is the identity, so reading
            // the raw i64 sum's sign made the 8-byte case a no-op — OF permanently 0, the same
            // silent-agreement shape as the carry bug this pair was found beside. The sign-compare
            // form is `OpBehaviorIntScarry`/`Sborrow` (opbehavior.cc:330/346): add overflows when
            // the operands share a sign and the result differs; subtract when they differ and the
            // result's sign is not the minuend's.
            "INT_SCARRY" => {
                let isize_ = op.ins.first().and_then(PArg::as_var).map_or(osize, |v| v.size);
                let (a, b) = (sa(0), sa(1));
                let s = sext(mask(a.wrapping_add(b) as u64, isize_), isize_);
                (((a < 0) == (b < 0)) && ((a < 0) != (s < 0))) as u64
            }
            "INT_SBORROW" => {
                let isize_ = op.ins.first().and_then(PArg::as_var).map_or(osize, |v| v.size);
                let (a, b) = (sa(0), sa(1));
                let s = sext(mask(a.wrapping_sub(b) as u64, isize_), isize_);
                (((a < 0) != (b < 0)) && ((a < 0) != (s < 0))) as u64
            }
