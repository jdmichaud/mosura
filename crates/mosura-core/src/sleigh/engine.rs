...
impl Labels {
    ...
    fn resolve(&self, ops: &mut [PcodeOp]) {
        for &(op_index, id, size) in &self.refs {
            ...
            // program property. Ghidra throws (`sleigh.cc:128`, "Reference to non-existant sleigh
            // label"); this builder has no error path, so the branch is emitted the way every
            // unresolvable op was before labels were resolved at all — with NO operands, which the
            // interpreter reads as a fall-through. Leaving the label id in the operand would make it
            // a displacement, and that is an invented target.
            let Some(Some(target)) = self.at.get(id as usize).copied() else {
                crate::debug::warn(format!("sleigh: reference to undefined label {id}; the branch is emitted without operands"));
                ops[op_index].ins.clear();
                continue;
            };
