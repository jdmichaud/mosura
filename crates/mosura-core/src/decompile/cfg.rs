...
pub fn build_cfg(f: &mut Funcdata) {
    ...
    // The out-of-range guard is NOT folded here. Ghidra keeps the guard CBRANCH live through the
    // whole simplification loop — `JumpBasic::foldInOneGuard`/`Funcdata::pushBranch` run inside
    // `ActionSwitchNorm` (coreaction.cc:4559), AFTER `matchModel` re-recovers the switch model on
    // the final graph. Folding at CFG-build time destroyed the guard comparison the norm-time
    // re-recovery pulls the switch variable's range back through (`analyze_guards`), leaving the
    // model unmatchable. See `jumpbasic::normalize_one`.
