...
//!
//! `cargo run --release -p mosura-core --example le_funcs [--bin <subject.exe>] [--dump] [±hex ...]`
...
fn main() {
    ...
    let path = argv
        ...
        .or_else(|| mosura_core::devcfg::subjects().first().map(|s| s.path.clone()))
        .unwrap_or_else(|| {
    ...
    let prog = mosura_core::analysis::analyze_le_file(&path).expect("analyze the subject");
