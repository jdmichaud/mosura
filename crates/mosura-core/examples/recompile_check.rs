...
use mosura_core::recompile::toolchain::{spec, Cached, CompileUnit, CompilerDriver, DriverRole, Toolchain};
use mosura_core::recompile::{
...
fn main() {
    // `--debug <spec>` configures the diagnostics (`mosura_core::debug`); it is taken out of the list first.
    let a = mosura_core::debug::from_args(std::env::args().skip(1).collect()).unwrap_or_else(|e| panic!("--debug: {e}"));
    let a = mosura_core::resources::from_args(a).unwrap_or_else(|e| panic!("{e}"));
    if a.len() < 5 {
        eprintln!(
    ...
    let (foreign_vas, foreign_stamp): (std::collections::HashSet<u64>, Option<String>) =
        match &foreign_file {
            Some(sf) => {
                ...
                let facts = mosura_core::analysis::foreign::extract_facts(&sprog);
                let conf = mosura_core::analysis::foreign::Confirmation::load(Path::new(sf)).expect("confirmation file");
                ...
                let vas = cls
                    .class
                    .iter()
                    .filter(|(_, c)| **c == mosura_core::analysis::foreign::Class::Foreign)
                    .map(|(va, _)| *va)
                    .collect();
                (vas, Some(stamp))
            }
    ...
    for r in &selected {
        ...
        let unit_flags: Vec<String> = if recover_flags {
            ...
            let insns = mosura_core::recompile::insn::normalize(
                LANG,
                &obytes,
                r.va,
                &mosura_core::recompile::insn::NoReloc,
            )
            .expect("language tables");
            profile.flags_for(&mosura_core::recompile::buildconfig::detect(&insns, sp, fp))
        } else {
            flags
                .get(&r.idx)
                .cloned()
    ...
    let orig_insns_of = |row: &Row| -> usize {
        ...
        mosura_core::recompile::insn::normalize(LANG, &obytes, row.va, &mosura_core::recompile::insn::NoReloc)
            .expect("language tables")
            .len()
    };
    ...
    for (row, out) in kept.iter().zip(outs.iter()) {
        ...
        let checked = match mosura_core::recompile::verify_with_image(LANG, &obytes, &subject, out.object.as_ref().unwrap(), &resolver, Some(&find_near)) {
            Ok(c) => c,
            Err(e) => {
        ...
        *census.entry(diff.verdict.as_str()).or_default() += 1;
        ...
        if div_path.is_some() {
            ...
            mosura_core::recompile::write_divergence_rows(&mut divs, &key, diff, orig, cnorm);
        }
        if verbose {
            println!("=== {} @ {:08x} : {} ===", row.name, row.va, diff.verdict.as_str());
            for op in &diff.ops {
                match op {
                    mosura_core::recompile::AlignOp::Pair { oi, ci, class } => println!(
                        "{} {:08x}  {:<36} | {:<36} {}",
    ...
    let gates_file = mosura_core::devcfg::subject_for(Path::new(bin)).and_then(|s| s.file("corpus-gates.tsv"));
