...
fn main() {
    ...
    if let Some(name) = std::env::args().nth(2) {
        ...
        match mosura_core::recompile::candidate::load_object_function(&data, &name, base, &resolver) {
            Ok(c) => {
                ...
                println!();
                match mosura_core::recompile::insn::normalize("x86:LE:32:default", &rl, base, &mosura_core::recompile::insn::NoReloc) {
                    Ok(insns) => {
