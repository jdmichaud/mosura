...
fn main() {
    ...
    let argv = mosura_core::resources::from_args(std::env::args().skip(1).collect()).unwrap_or_else(|e| panic!("{e}"));
    ...
    match mosura_core::analysis::native_loader_name(&data) {
    ...
    let program = if native {
        mosura_core::analysis::analyze_native_file(&path)
    } else if le {
        mosura_core::analysis::analyze_le_file(&path)
    } else {
        mosura_core::analysis::analyze_file_as(&path, cspec.as_deref())
    }
    ...
    match mosura_core::lang::resolve_cspec(&program.language_id, &program.compiler_spec_id) {
    ...
    let service = mosura_core::analysis::fid::query::FidQueryService::load_matching_resources(
        &program.language_id,
    ...
    let results = mosura_core::analysis::fid::analyzer::search_program(&program, &service);
