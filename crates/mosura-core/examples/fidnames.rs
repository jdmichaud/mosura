...
//! ```
//!
//! `--le` selects [`analyze_le_file`](mosura_core::analysis::analyze_le_file) for a DOS/4GW-bound
...
fn main() {
    ...
    let program = if native {
        mosura_core::analysis::analyze_native_file(&bin)
    } else if le {
        mosura_core::analysis::analyze_le_file(&bin)
    } else {
        mosura_core::analysis::analyze_file(&bin)
    }
    .expect("analyze the binary");

    let service = match &db_dir {
        Some(dir) => mosura_core::analysis::fid::query::FidQueryService::load_matching(
            dir,
            &program.language_id,
            &program.compiler_spec_id,
        ),
        None => mosura_core::analysis::fid::query::FidQueryService::load_matching_resources(
            &program.language_id,
            &program.compiler_spec_id,
        ),
    };
