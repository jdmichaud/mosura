...
use mosura_core::datatest;

fn main() {
    let args = mosura_core::debug::from_args(std::env::args().collect()).unwrap_or_else(|e| panic!("--debug: {e}"));
    let args = mosura_core::resources::from_args(args).unwrap_or_else(|e| panic!("{e}"));
    let stem = args.get(1).expect("fixture stem");
    let path = mosura_core::paths::datatests_dir().join(format!("{stem}.xml"));
    ...
    let (spec, ctx) = mosura_core::lang::load_cached(lang_id).expect("fixture language loads");
