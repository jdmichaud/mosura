...
pub static ALL: &[&Schema] = &[&TOOLCHAIN_EVIDENCE, &FID_NAMES, &OPS, &SCHEMA, &IDENTIFY, &PROGRAM_SUMMARY, &TABLES, &BYTES, &INSTRUCTIONS, &PCODE, &PROTOTYPE, &JUMPTABLES, &CALLS, &GLOBAL_WIDTHS, &EMIT_REPORT, &OPTION_REGISTRY, &FILES, &DATA, &LANGUAGES, &REGISTERS, &EMIT_AXES, &EMIT_ARMS, &TOOLCHAIN_SPECS, &TOOLCHAINS, &CHECK, &EMISSION, &BUILDCONFIG, &VERDICTS, &DIVERGENCES, &DIFF, &GATES, &COMPARE, &ROUNDS, &COMPILE_UNITS, &COMPILE_OUTPUTS];
